...
public class ModelCheckingDriver {
	...
	/*
	 * Logic as of now for widgets like number picker, and maybe datepicker too
	 * we'll go with default values We have not handled Popup Menu, PopupWindow,
	 * ProgressBar(a subclass of View) specially , which is not a subclass of
	 * View
	 * 
	 * Spinners are not like lists. They support only listeners that a View
	 * supports. nothing more
	 * 
	 * TextView maintains a list of TextWatcher listeners Toast widget is read
	 * only. So ignore them
	 */

	// Note: the current logic does not collect details on various kind of
	// events. Ignore
	// corresponding fields.

	// events common to all Views
	// Note that ImageView, ImageButton are subclasses of views and do not have
	// any listeners other
	// than that of a View
	public static final int FOCUS_CHANGE = 1; // this need not be specially
												// triggered
	...
	// not tested unless needed, for e.g., list item has too many items and one
	// needs to scroll, or we are in scrollview layout
	...
	// for lists, spinners, adapterViews, AutoCompleteTextViews (to handle drop
	// down options)
	...
	public static final int FLAG_INTENT = 3;

	public static final String TAG = "Android-Bug-Checker";
	public static final String TAG_DEBUG = "ABC-DEBUG";

	// events like BACK button click, screen rotation etc. will be classified as
	// UI
	...
	public static final int MODE = EXPLORE; // todo: this should be read from a
											// file
	...
	// set to true when app is waiting to be killed by Android bug-checker
	// server
	...
	// counts the period of inactivity after an event trigger and continues
	// normally after the limit is reached
	...
	// setDate(long, ture, true) for calendarView; find min-max before settin
	// date and
	// set a date which is mid
	public static final int EVENT_SET_DATE_ANIMATE = 6;

	// (int year, int month, int day) for datePicker, get min and max and
	// convert this no. to day/month/year
	...
	// setValue(int) for NumberPicker perform getMax and getMin before
	...
	// setQuery(String, true) to set a query. use data given by retrieveText as
	// query
	...
	/*
	 * List of View types Used to classify a given view object and apply data
	 * view heuristic
	 */
	...
	// chronometer is a read only object, it can be set-reset-paused-started if
	// user has provided
	// other buttons to interact with chronometer. So, dont call start(),
	// stop(),setBase() methods
	...
	// for CheckedTextView, CheckBox, ToggleButton and Switch ask isChecked()
	// and then call toggle()
	// but calling toggle() is same as clicking it..so no need to explicitly
	// call toggle, if you find
	...
	// Note: DigitalClock and AnalogClock widgets are only meant to show system
	// time. If a view
	// is of this type, dont store its children, perform click/long click on
	...
	// ViewAnimator - can have many child views which are reached by
	// programmatically calling
	// showNext(), showPrevious() (check you dont go beyond boundary), or by
	// performing
	// click, fling etc. which will call showPrev() etc. As of now we will use
	// clicks to go
	...
	// Use setValue() to set a number, perform getMinValue() and getMaxValue()
	// before setting
	...
	// Progress bar's progress cant be set by the user..if its clickable or long
	// clickable perform it
	...
	// RatingBar - use methods like setRating and getNumStars to set valid
	// rating
	public static final int VIEW_RATING_BAR = 31;

	// Use scrollTo() to perform scroll.
	// or perform view.requestFocus() and only then perform any event on the
	// view
	...
	// StackView - do getChildCount() for safety and then perform showNext() or
	// showPrevios()
	public static final int VIEW_STACK_VIEW = 35;

	public static final int VIEW_TAB_HOST = 37; // this is just a container of
												// tabs and frameViews
	...
	private static boolean fireIntentCheck = true;

	public void setFireIntent(boolean flag) {
		ModelCheckingDriver.fireIntentCheck = flag;
	}

	private static int intentCount = 39;
	private static int pendingIntentCount = 0;
	...
	public static final int INTENT_TYPE_SYSTEM = 3;

	// INTENT action
	public static final int INTENT_AUDIO_BECOMING_NOISY = 0;
	public static final int INTENT_ACTION_SHUTDOWN = 1;
	public static final int INTENT_ACTION_POWER_CONNECTED = 2;
	public static final int INTENT_ACTION_POWER_DISCONNECTED = 3;
	public static final int INTENT_AIRPLANE_MODE = 4;
	public static final int INTENT_BATTERY_CHANGED = 5;
	public static final int INTENT_BATTERY_LOW = 6;
	public static final int INTENT_BATTERY_OKAY = 7;
	public static final int INTENT_BOOT_COMPLETED = 8;
	public static final int INTENT_DATE_CHANGED = 9;
	public static final int INTENT_DEVICE_STORAGE_LOW = 10;
	public static final int INTENT_INPUT_METHOD_CHANGED = 11;
	public static final int INTENT_MEDIA_EJECT = 12;
	public static final int INTENT_MEDIA_MOUNTED = 13;
	public static final int INTENT_MEDIA_REMOVED = 14;
	public static final int INTENT_MEDIA_SCANNER_FINISHED = 15;
	public static final int INTENT_MEDIA_SHARED = 16;
	public static final int INTENT_MEDIA_UNMOUNTABLE = 17;
	public static final int INTENT_MEDIA_UNMOUNTED = 18;
	public static final int INTENT_NEW_OUTGOING_CALL = 19;
	public static final int INTENT_PACKAGE_ADDED = 20;
	public static final int INTENT_PACKAGE_CHANGED = 21;
	public static final int INTENT_PACKAGE_DATA_CLEARED = 22;
	public static final int INTENT_PACKAGE_FIRST_LAUNCH = 23;
	public static final int INTENT_PACKAGE_FULLY_REMOVED = 24;
	public static final int INTENT_PACKAGE_INSTALL = 25;
	public static final int INTENT_PACKAGE_REMOVED = 26;
	public static final int INTENT_PACKAGE_REPLACED = 27;
	public static final int INTENT_PACKAGE_RESTARTED = 28;
	public static final int INTENT_PHONE_STATE = 29;// **** phone restarts
	public static final int INTENT_REBOOT = 30;
	public static final int INTENT_SCREEN_OFF = 31;
	public static final int INTENT_SCREEN_ON = 32;
	public static final int INTENT_TIMEZONE_CHANGED = 33;
	public static final int INTENT_TIME_SET = 34;
	public static final int INTENT_TIME_TICK = 35;
	public static final int INTENT_USER_PRESENT = 36;
	public static final int INTENT_LOCALE_CHANGED = 37;
	public static final int INTENT_CONFIGURATION_CHANGED = 38;

	public static HashMap<String, Integer> intentClassMap = new HashMap<String, Integer>();
	public static ArrayList<String> intentActionList = new ArrayList<String>();
	// public static ArrayList<String> intentCategoryList = new
	// ArrayList<String>();
	...
	static {
		intentActionList.add("android.media.AUDIO_BECOMING_NOISY");
		intentActionList.add("android.intent.action.ACTION_SHUTDOWN");
		intentActionList.add("android.intent.action.ACTION_POWER_CONNECTED");
		intentActionList.add("android.intent.action.ACTION_POWER_DISCONNECTED");
		intentActionList.add("android.intent.action.AIRPLANE_MODE");
		intentActionList.add("android.intent.action.BATTERY_CHANGED");
		intentActionList.add("android.intent.action.BATTERY_LOW");
		intentActionList.add("android.intent.action.BATTERY_OKAY");
		intentActionList.add("android.intent.action.BOOT_COMPLETED");
		intentActionList.add("android.intent.action.DATE_CHANGED");
		intentActionList.add("android.intent.action.DEVICE_STORAGE_LOW");
		intentActionList.add("android.intent.action.INPUT_METHOD_CHANGED");
		intentActionList.add("android.intent.action.MEDIA_EJECT");
		intentActionList.add("android.intent.action.MEDIA_MOUNTED");
		intentActionList.add("android.intent.action.MEDIA_REMOVED");
		intentActionList.add("android.intent.action.MEDIA_SCANNER_FINISHED");
		intentActionList.add("android.intent.action.MEDIA_SHARED");
		intentActionList.add("android.intent.action.MEDIA_UNMOUNTABLE");
		intentActionList.add("android.intent.action.MEDIA_UNMOUNTED");
		intentActionList.add("android.intent.action.NEW_OUTGOING_CALL");
		intentActionList.add("android.intent.action.PACKAGE_ADDED");
		intentActionList.add("android.intent.action.PACKAGE_CHANGED");
		intentActionList.add("android.intent.action.PACKAGE_DATA_CLEARED");
		intentActionList.add("android.intent.action.PACKAGE_FIRST_LAUNCH");
		intentActionList.add("android.intent.action.PACKAGE_FULLY_REMOVED");
		intentActionList.add("android.intent.action.PACKAGE_INSTALL");
		intentActionList.add("android.intent.action.PACKAGE_REMOVED");
		intentActionList.add("android.intent.action.PACKAGE_REPLACED");
		intentActionList.add("android.intent.action.PACKAGE_RESTARTED");
		intentActionList.add("android.intent.action.PHONE_STATE");
		intentActionList.add("android.intent.action.REBOOT");
		intentActionList.add("android.intent.action.SCREEN_OFF");
		intentActionList.add("android.intent.action.SCREEN_ON");
		intentActionList.add("android.intent.action.TIMEZONE_CHANGED");
		intentActionList.add("android.intent.action.TIME_SET");
		intentActionList.add("android.intent.action.TIME_TICK");
		intentActionList.add("android.intent.action.USER_PRESENT");
		intentActionList.add("android.intent.action.LOCALE_CHANGED");
		intentActionList.add("android.intent.action.CONFIGURATION_CHANGED");
	}
	public static int DEPTH_LIMIT = 5; // this should be supplied by a file and
	...
	public void initKeyPressEventsAndRotateScreen(SQLiteDatabase db) {
		...
		Looper.mcd.BACK_CLICK_EVENT_ID = (int) db.insert(McdDB.TABLE_UI_EVENT,
				null, values);

		values = new ContentValues();
		values.put(McdDB.COLUMN_VIEW_ID, -1);
		values.put(McdDB.COLUMN_UI_EVENT_TYPE, EVENT_MENU_CLICK);
		Looper.mcd.MENU_CLICK_EVENT_ID = (int) db.insert(McdDB.TABLE_UI_EVENT,
				null, values);

		values = new ContentValues();
		values.put(McdDB.COLUMN_VIEW_ID, -1);
		values.put(McdDB.COLUMN_UI_EVENT_TYPE, EVENT_ROTATE_SCREEN);
		Looper.mcd.ROTATE_SCREEN_EVENT_ID = (int) db.insert(
				McdDB.TABLE_UI_EVENT, null, values);
	}

	public void getKeyPressEventsAndRotateScreenIds(SQLiteDatabase db) {
		Cursor cursor = db.query(McdDB.TABLE_UI_EVENT,
				new String[] { McdDB.COLUMN_ID }, McdDB.COLUMN_VIEW_ID
						+ " = -1 AND " + McdDB.COLUMN_UI_EVENT_TYPE + " = ?",
				new String[] { String.valueOf(EVENT_BACK) }, null, null, null);
		cursor.moveToFirst();
		BACK_CLICK_EVENT_ID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_ID));

		cursor.close();
		cursor = null;
		cursor = db.query(McdDB.TABLE_UI_EVENT,
				new String[] { McdDB.COLUMN_ID }, McdDB.COLUMN_VIEW_ID
						+ " = -1 AND " + McdDB.COLUMN_UI_EVENT_TYPE + " = ?",
				new String[] { String.valueOf(EVENT_MENU_CLICK) }, null, null,
				null);
		cursor.moveToFirst();
		MENU_CLICK_EVENT_ID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_ID));
		...
		cursor = db.query(McdDB.TABLE_UI_EVENT,
				new String[] { McdDB.COLUMN_ID }, McdDB.COLUMN_VIEW_ID
						+ " = -1 AND " + McdDB.COLUMN_UI_EVENT_TYPE + " = ?",
				new String[] { String.valueOf(EVENT_ROTATE_SCREEN) }, null,
				null, null);
		cursor.moveToFirst();
		ROTATE_SCREEN_EVENT_ID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_ID));
		cursor.close();
	...
	public HashSet<Integer> getSpecialEventsForView(int viewType) {
		...
		case VIEW_CALENDAR_VIEW:
			eventCodeSet.add(EVENT_SET_DATE_ANIMATE);
			break;
		case VIEW_DATE_PICKER:
	...
	// this maps any given view class to a known view type (by recursively
	// querying superclass).
	// If no matching found keeps it as View and does not classify it as a data
	// widget
	public int getSimplifiedClassOfView(Class viewClass) throws McdException {
		...
		if (clazz == -1) {
			mcdRaiseException(
					"Android Bug-checker returned an invalid class of"
							+ " type -1", null);
		}
		return clazz;
	}
	...
	// match given views of a popup window with existing one in the database
	// one-on-one
	public boolean matchPopupViews(int popupID, View v, int parentID,
			int parentClass, String trait, int guiID, SQLiteDatabase database)
			throws McdException {
		boolean flag = true;
		int viewClass = getSimplifiedClassOfView(v.getClass());
		// compare trait and class
		if (parentClass == viewClass
				&& guiID == v.getId()
				&& ((guiID != -1) ? true : (trait != null) ? (trait
						.equals(getTraitOfView(v)))
						: (getTraitOfView(v) == null))) {
			if (!isIgnoreChildrenViews(viewClass)) {
				Cursor cursor = database.query(
						McdDB.TABLE_VIEW_NODE,
						new String[] { McdDB.COLUMN_ID, McdDB.COLUMN_VIEW_TYPE,
								McdDB.COLUMN_TRAIT, McdDB.COLUMN_GUI_ID },
						McdDB.COLUMN_POPUP_ID + " = ? AND "
								+ McdDB.COLUMN_PARENT_ID + " = ?",
						new String[] { String.valueOf(popupID),
								String.valueOf(parentID) }, null, null,
						McdDB.COLUMN_RELATIVE_ID + " ASC");
				if (ViewGroup.class.isInstance(v)) {
					if (((ViewGroup) v).getChildCount() == cursor.getCount()) {
						if (cursor.moveToFirst()) {
							for (int i = 0; i < cursor.getCount(); i++) {
								flag = matchPopupViews(
										popupID,
										((ViewGroup) v).getChildAt(i),
										cursor.getInt(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_ID)),
										cursor.getInt(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)),
										cursor.getString(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_TRAIT)),
										cursor.getInt(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)),
										database);
								if (flag == false)
	...
	// we create entry for identical popups belonging to different activity
	// objects
	// but do not explore as per current logic

	// perform one to one comparison between existing ViewGroups's children and
	// database entry

	// viewID - view on which when event eventType is performed to get a popup
	// whose
	// rootview is v
	// v is not null

	// returns a pair of matching popupID and a set of matching screenIds where
	// viewRootLevel
	// of this popupWindow also matches
	public Pair<Integer, HashSet<Integer>> doIdenticalPopupsExist(int viewID,
			int eventType, int viewRootLevel, View v, SQLiteDatabase database)
			throws McdException {
		boolean flag = false;
		...
		Cursor cursor = database.query(
				...
				McdDB.COLUMN_VIEW_ID + " = ? AND " + McdDB.COLUMN_EVENT_TYPE
						+ " = ?",
				new String[] { String.valueOf(viewID),
						String.valueOf(eventType) }, null, null, null);

		if (cursor.moveToFirst()) {
			for (int i = 0; i < cursor.getCount(); i++) {
				int popupID = cursor.getInt(cursor
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				// get row corresponding to root view of popup window
				Cursor cursorTmp = database.query(
						McdDB.TABLE_VIEW_NODE,
						new String[] { McdDB.COLUMN_ID, McdDB.COLUMN_VIEW_TYPE,
								McdDB.COLUMN_GUI_ID },
						McdDB.COLUMN_POPUP_ID + " = ? AND "
						...
						new String[] { String.valueOf(popupID),
								String.valueOf(-1), String.valueOf(0) }, null,
						null, null);
				if (cursorTmp.getCount() != 1) {
					mcdRaiseException(
							"Android Bug-checker found a screen/popup window "
									+ "with more than one root view", database);
				}

				cursorTmp.moveToFirst();
				int tmpViewID = cursorTmp.getInt(cursorTmp
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				int viewType = cursorTmp.getInt(cursorTmp
						.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE));
				int guiID = cursorTmp.getInt(cursorTmp
						.getColumnIndexOrThrow(McdDB.COLUMN_GUI_ID));
				cursorTmp.close();
				cursorTmp = null;
				...
				if (flag) { // found a matching popup
					Cursor popupScreens = database.query(
							McdDB.TABLE_POPUPS_SCREEN,
							new String[] { McdDB.COLUMN_SCREEN_ID },
							McdDB.COLUMN_POPUP_ID + " = ? AND "
									+ McdDB.COLUMN_VIEWROOT_LEVEL + " = ?",
							new String[] { String.valueOf(popupID),
									String.valueOf(viewRootLevel) }, null,
							null, null);
					if (popupScreens.moveToFirst()) {
						for (int k = 0; k < popupScreens.getCount(); k++) {
							matchingScreenIds
									.add(popupScreens.getInt(popupScreens
											.getColumnIndexOrThrow(McdDB.COLUMN_SCREEN_ID)));
							popupScreens.moveToNext();
						}
					} else {
						mcdRaiseException(
								"Android Bug-checker hit a popup window"
										+ " belonging to no screen", database);
					}
					matchingPopup = new Pair<Integer, HashSet<Integer>>(
							popupID, matchingScreenIds);
					popupScreens.close();
					popupScreens = null;
					break;
				}
	...
	public boolean matchScreenViews(int viewID, View v, SQLiteDatabase database)
			throws McdException {
		...
		if (!isIgnoreChildrenViews(getSimplifiedClassOfView(v.getClass()))) {
			Cursor cursor = database.query(McdDB.TABLE_VIEW_NODE, new String[] {
					McdDB.COLUMN_ID, McdDB.COLUMN_VIEW_TYPE,
					McdDB.COLUMN_GUI_ID, McdDB.COLUMN_TRAIT },
					McdDB.COLUMN_POPUP_ID + " = ? AND "
							+ McdDB.COLUMN_PARENT_ID + " = ?", new String[] {
							String.valueOf(-1), String.valueOf(viewID) }, null,
					null, McdDB.COLUMN_RELATIVE_ID + " ASC");
			if (ViewGroup.class.isInstance(v)) {
				if (((ViewGroup) v).getChildCount() == cursor.getCount()) {
					if (cursor.moveToFirst()) {
						for (int i = 0; i < cursor.getCount(); i++) {
							String trait = cursor.getString(cursor
									.getColumnIndexOrThrow(McdDB.COLUMN_TRAIT));
							View tmpView = ((ViewGroup) v).getChildAt(i);
							if (cursor
									.getInt(cursor
											.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)) == getSimplifiedClassOfView(tmpView
									.getClass())
									&& cursor
											.getInt(cursor
													.getColumnIndexOrThrow(McdDB.COLUMN_GUI_ID)) == tmpView
											.getId()
									&& ((tmpView.getId() != -1) ? true
											: (trait != null) ? (trait
													.equals(getTraitOfView(tmpView)))
													: (getTraitOfView(tmpView) == null))) {

								doesMatch = matchScreenViews(
										cursor.getInt(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_ID)),
										((ViewGroup) v).getChildAt(i), database);
	...
	public int findIdenticalActivityScreen(View v, String activity,
			SQLiteDatabase database) throws McdException {
		int screenID = -1;
		Cursor cursor = database.query(McdDB.TABLE_SCREEN,
				new String[] { McdDB.COLUMN_ID }, McdDB.COLUMN_ACTIVITY_NAME
						+ " = ?", new String[] { activity }, null, null, null);

		if (cursor.moveToFirst()) {
			for (int i = 0; i < cursor.getCount(); i++) {
				Cursor tmpCursor = database
						.query(McdDB.TABLE_VIEW_NODE,
								new String[] { McdDB.COLUMN_ID,
										McdDB.COLUMN_VIEW_TYPE,
										McdDB.COLUMN_GUI_ID },
								McdDB.COLUMN_SCREEN_ID + " = ? AND "
										+ McdDB.COLUMN_RELATIVE_ID
										+ " = ? AND " + McdDB.COLUMN_PARENT_ID
										+ " = ? AND " + McdDB.COLUMN_POPUP_ID
										+ " = ?",
								new String[] {
				...
				if (tmpCursor.getCount() != 1) {
					try {
						throw new McdException(
								"Android Bug-checker hit a screen with no root view or"
				...
				int viewID = tmpCursor.getInt(tmpCursor
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				int viewType = tmpCursor.getInt(tmpCursor
						.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE));
				int guid = tmpCursor.getInt(tmpCursor
						.getColumnIndexOrThrow(McdDB.COLUMN_GUI_ID));
				tmpCursor.close();
				tmpCursor = null;

				boolean matchFlag = false;
				if (viewType == getSimplifiedClassOfView(v.getClass())
						&& guid == v.getId()) {
	...
	public void storeChildrenViews(ViewGroup parent, long parentID,
			long screenID, int popupID, int isAncestorDataView,
			SQLiteDatabase database) throws McdException {
		ContentValues values;
		int tmpIsAncestorData;

		for (int i = 0; i < parent.getChildCount(); i++) {
			tmpIsAncestorData = 1; // if ancestor is not a data case is handled
									// below
			int viewClass = getSimplifiedClassOfView(parent.getChildAt(i)
					.getClass());
			...
			if (!(viewID > -1)) {
				mcdRaiseException(
						"Android Bug-checker returned a database row for ViewBode"
								+ " with primary key being -1", database);
			}

			if (ViewGroup.class.isInstance(parent.getChildAt(i))
					&& !isIgnoreChildrenViews(viewClass)) {
				storeChildrenViews((ViewGroup) parent.getChildAt(i), viewID,
						screenID, popupID, tmpIsAncestorData, database);
			}
		}
	}

	// store views on a screen in the database
	// returns id of ui_env row created. This can be used to store in the
	// pathNode
	public int[] storeScreenElements(View rootView, int activityID,
			String activity, int popupID, long hash, SQLiteDatabase database)
			throws McdException {
		int ui_envID = -1;
		int screenID = -1;
		ContentValues values = null;
		int[] result = new int[2];
		// creation of screen entry and ui_env entry for popup windows is
		// handled separately
		if (popupID == -1) {
			values = new ContentValues();
			values.put(McdDB.COLUMN_ACTIVITY_NAME, activity);
			values.put(McdDB.COLUMN_SCREEN_HASH, 0);
			screenID = (int) database.insert(McdDB.TABLE_SCREEN, null, values);

			if (!(screenID > -1)) {
				mcdRaiseException(
						"Android Bug-checker returned a datbase row for"
		...
		if (ViewGroup.class.isInstance(rootView)
				&& !isIgnoreChildrenViews(viewClass)) {
			storeChildrenViews((ViewGroup) rootView, parentID, screenID,
					popupID, 0, database);
		}
	...
	public int createDummyPathNode(int uiEnvID, SQLiteDatabase database)
			throws McdException {
		int pathNodeID;
		ContentValues values = new ContentValues();
		...
		if (!(pathNodeID > -1)) {
			mcdRaiseException(
					"Android Bug-checker returned a database row for PathNode"
	...
	public void updatePathNodeWithEvent(int pathNodeID, int eventID,
			int eventType, SQLiteDatabase database) {
		ContentValues values = new ContentValues();
		values.put(McdDB.COLUMN_EVENT_ID, eventID);
		values.put(McdDB.COLUMN_EVENT_TYPE, eventType);
		database.update(McdDB.TABLE_PATH_NODE, values,
				McdDB.COLUMN_ID + " = ?",
	...
	public void addEventToUnexploredList(long eventID, int eventType,
			int pathNodeID, int priority, SQLiteDatabase database)
			throws McdException {
		...
		long insertID = database.insert(McdDB.TABLE_UNEXPLORED_EVENTS, null,
				values);

		if (!(insertID > -1)) {
			mcdRaiseException("Android Bug-checker returned a database row for"
					+ " for UnexploredEvents table with primary key -1",
					database);
		}
	}
	...
	public boolean isEditTextWritable(EditText et) {
		...
		// write code to find out if editText has not been made read only. for
		// this
		// some library instrumentation may be needed which has not yet been
		// done.
	...
	public int storeViewEvents(View v, int viewID, int pathNodeID,
			int parentViewType, SQLiteDatabase database) throws McdException {
		ContentValues values;
		long eventID;
		Cursor cursor;
		int priority;
		...
		if (v.getVisibility() == View.VISIBLE && v.isEnabled()) {
			if (v.isClickable()
					...
					&& !(parentViewType == VIEW_LIST_VIEW
							|| parentViewType == VIEW_SPINNER
							|| parentViewType == VIEW_ABS_LIST_VIEW
							|| parentViewType == VIEW_ABS_SPINNER || parentViewType == VIEW_EXPANDABLE_LIST_VIEW)) {
				cursor = database
						.query(McdDB.TABLE_UI_EVENT,
								new String[] { McdDB.COLUMN_ID },
								McdDB.COLUMN_VIEW_ID + " = ? AND "
										+ McdDB.COLUMN_EVENT_TYPE + " = ?",
								new String[] { String.valueOf(viewID),
										String.valueOf(EVENT_CLICK) }, null,
								null, null);
				if (!cursor.moveToFirst()) {
					values = new ContentValues();
					values.put(McdDB.COLUMN_VIEW_ID, viewID);
					values.put(McdDB.COLUMN_UI_EVENT_TYPE, EVENT_CLICK);
					eventID = database.insert(McdDB.TABLE_UI_EVENT, null,
							values);
					if (!(eventID > -1)) {
						mcdRaiseException(
								"Android Bug-checker returned a database row for"
										+ " UiEvent table with primary key -1",
								database);
					}
				} else {
				...
				cursor.close();
				cursor = null;
				// we do not add click and long click directly on list/spinner
				// but on their children
				if (!(viewType == VIEW_LIST_VIEW || viewType == VIEW_SPINNER
						|| viewType == VIEW_ABS_LIST_VIEW
						|| viewType == VIEW_ABS_SPINNER || viewType == VIEW_EXPANDABLE_LIST_VIEW)) {
					cursor = database.query(
							McdDB.TABLE_UI_EVENT_RELATIVE_PRIORITY,
							new String[] { McdDB.COLUMN_EVENT_PRIORITY },
							McdDB.COLUMN_VIEW_TYPE + " = ? AND "
									+ McdDB.COLUMN_UI_EVENT_TYPE + " = ?",
							new String[] { String.valueOf(viewType),
									String.valueOf(EVENT_CLICK) }, null, null,
							null);
					if (cursor.moveToFirst())
						priority = cursor
								.getInt(cursor
										.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_PRIORITY));
					else
						priority = 5; // some default value

					addEventToUnexploredList(eventID, UI_EVENT, pathNodeID,
							priority, database);
					cursor.close();
					cursor = null;
				}
			} else if (parentViewType == VIEW_LIST_VIEW
					|| parentViewType == VIEW_SPINNER
					|| parentViewType == VIEW_ABS_LIST_VIEW
					|| parentViewType == VIEW_ABS_SPINNER
					|| parentViewType == VIEW_EXPANDABLE_LIST_VIEW) {
				Cursor tmp = database.rawQuery("SELECT " + McdDB.COLUMN_ID
						+ " FROM " + McdDB.TABLE_UI_EVENT + " WHERE "
						+ McdDB.COLUMN_UI_EVENT_TYPE + " = ? AND "
						+ McdDB.COLUMN_VIEW_ID + " IN (SELECT "
						+ McdDB.COLUMN_PARENT_ID + " FROM "
			...
			if (v.isLongClickable()
				...
				cursor = database.query(
						...
						McdDB.COLUMN_VIEW_ID + " = ? AND "
								+ McdDB.COLUMN_EVENT_TYPE + " = ?",
						new String[] { String.valueOf(viewID),
								String.valueOf(EVENT_LONG_CLICK) }, null, null,
						null);
				if (!cursor.moveToFirst()) {
					values = new ContentValues();
					values.put(McdDB.COLUMN_VIEW_ID, viewID);
				...
				if (!(viewType == VIEW_LIST_VIEW || viewType == VIEW_SPINNER
					...
					cursor = database.query(
							...
							McdDB.COLUMN_VIEW_TYPE + " = ? AND "
									+ McdDB.COLUMN_UI_EVENT_TYPE + " = ?",
							new String[] { String.valueOf(viewType),
									String.valueOf(EVENT_LONG_CLICK) }, null,
							null, null);
					if (cursor.moveToFirst())
						priority = cursor
								.getInt(cursor
			...
			if (v.isClickable()) {
					...
					for (Integer eventCode : splEvents) {
						cursor = database.query(
								...
								McdDB.COLUMN_VIEW_ID + " = ? AND "
										+ McdDB.COLUMN_EVENT_TYPE + " = ?",
								new String[] { String.valueOf(viewID),
										String.valueOf(eventCode) }, null,
								null, null);
						if (!cursor.moveToFirst()) {
							values = new ContentValues();
							values.put(McdDB.COLUMN_VIEW_ID, viewID);
							values.put(McdDB.COLUMN_UI_EVENT_TYPE, eventCode);
							eventID = database.insert(McdDB.TABLE_UI_EVENT,
									null, values);
							if (!(eventID > -1)) {
								mcdRaiseException(
										"Android Bug-checker returned a database row for"
				...
				} else if (EditText.class.isInstance(v)
						&& isEditTextWritable((EditText) v)) {
					cursor = database.query(
							McdDB.TABLE_UI_EVENT,
							new String[] { McdDB.COLUMN_ID },
							McdDB.COLUMN_VIEW_ID + " = ? AND "
									+ McdDB.COLUMN_EVENT_TYPE + " = ?",
							new String[] { String.valueOf(viewID),
									String.valueOf(EVENT_SET_TEXT) }, null,
							null, null);
					if (!cursor.moveToFirst()) {
						values = new ContentValues();
						values.put(McdDB.COLUMN_VIEW_ID, viewID);
	...
	public void storeUIEvents(View v, int viewID, int pathNodeID,
			int parentViewType, SQLiteDatabase database) throws McdException {
		int viewType = storeViewEvents(v, viewID, pathNodeID, parentViewType,
				database);

		if (ViewGroup.class.isInstance(v)
				&& !isIgnoreChildrenViews(getSimplifiedClassOfView(v.getClass()))) {
			Cursor cursor = database.query(McdDB.TABLE_VIEW_NODE,
					new String[] { McdDB.COLUMN_ID }, McdDB.COLUMN_PARENT_ID
							+ " = ?", new String[] { String.valueOf(viewID) },
					null, null, McdDB.COLUMN_RELATIVE_ID + " ASC");

			if (cursor.getCount() != ((ViewGroup) v).getChildCount()) {
				mcdRaiseException(
						"Android Bug-checker hit a view whose children count did not"
								+ " match that stored in database for that view",
						database);
			}

			if (cursor.moveToFirst()) {
				for (int i = 0; i < cursor.getCount(); i++) {
					storeUIEvents(((ViewGroup) v).getChildAt(i),
							cursor.getInt(cursor
									.getColumnIndexOrThrow(McdDB.COLUMN_ID)),
							pathNodeID, viewType, database);
					cursor.moveToNext();
				}
			}
	...
	public void storeIntent(int intentID, int componentType,
		...
		values.put(McdDB.COLUMN_INTENT_TYPE, componentType);
		if (mime != "")
			values.put(McdDB.COLUMN_INTENT_DATA, mime);
		values.put(McdDB.COLUMN_INTENT_AVAIL_FLAG, flag);
		values.put(McdDB.COLUMN_INTENT_COMPONENT, componentName);
		// values.put(McdDB.COLUMN_INTENT_ACTION,
		// ic.intentFilters.get(0).intentActions.get(0));
	...
	public void storeCategories(int intentId, String category,
		...
		values = new ContentValues();
		values.put(McdDB.COLUMN_INTENT_ID, intentId);
		values.put(McdDB.COLUMN_INTENT_CATEGORY, category);
	...
	public boolean triggerIntent(String Action, String[] intentCategories,
		...
		Log.v("Trigger Intent called : ", "Category : " + categoryString);
		...
		if (dataUri != null) {
			if (dataUri.toLowerCase().contains("audio")
				...
				if (dataUri.toLowerCase().contains("http"))
					...
					data = " -d /mnt/sdcard/test.mp3";
			} else if (dataUri.toLowerCase().contains("video")) {
				if (dataUri.toLowerCase().contains("http"))
				...
				if (dataUri.toLowerCase().contains("http"))
					...
					data = " -d /mnt/sdcard/test.jpg";
			}
		}

		Log.v("Trigger Intent called : ", "DataMime : " + data);
		...
		Log.v("Trigger Intent called : ", "Cmd : " + targetBroadCast);

		/*
		 * Thread triggerIntentThread = new Thread(new
		 * ModelCheckingClient("triggerIntent", 8889, targetBroadCast, this));
		 * triggerIntentThread.start();
		 */
		...
		intent.putExtra("errCode", FLAG_INTENT);
		intent.putExtra("command", targetBroadCast);
	...
	// store events corresponding to current screen in the database if not
	// already present
	// and add it to the unexplored list corresponding to the pathNode
	public void storeCurrentUIEvents(View rootView, int viewID, int pathNodeID,
			SQLiteDatabase db) throws McdException {
		storeUIEvents(rootView, viewID, pathNodeID, -1, db);
	}

	// for screen hash, if an object has a GUID use that in summation otherwise
	// the
	// hash value, because in many cases guid gets maintained on a refresh or
	// revisit of a screen by
	...
	public long computeScreenHash(ViewGroup vg, long sum) throws McdException {
		if (vg != null) {
			for (int i = 0; i < vg.getChildCount(); i++) {
				...
				if (ViewGroup.class.isInstance(v)
						&& !isIgnoreChildrenViews(getSimplifiedClassOfView(v
								.getClass()))) {
	...
	public int getMappedInputType(int rawType) {
		...
		// switch case with the list of input types for which you have
		// customized input
		if ((rawType & 0x0000000f) == InputType.TYPE_CLASS_PHONE) {
			...
			int tmpRawType = rawType & 0x000000f0; // to ignore flag fields of
													// bit-mask
			if ((tmpRawType == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS)
				...
				if ((rawType & InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS) != 0) {
					...
					// our default input is Testdata, a 8 digit no. and _a*$hj
					// kind of
					// word
	...
	public String getTextDataStoredForPathNode(int pathNodeID,
			SQLiteDatabase database) {
		String data = null;
		Cursor cursor = database.query(McdDB.TABLE_QUICK_NODE_DATA_MAP,
				new String[] { McdDB.COLUMN_DATA }, McdDB.COLUMN_NODE_ID
						+ " = ?", new String[] { String.valueOf(pathNodeID) },
				null, null, null);

		if (cursor.moveToFirst()) {
			data = cursor.getString(cursor
					.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
		}
		cursor.close();
	...
	// uses a storage to keep track of strings to be fed to an
	// editText/searchView etc.
	// check the view's type
	// pathNodeId is used to keep track of data already provided if there are
	// options available for
	// the view and is useful during backtracking
	public String retrieveNextDataForView(View v, int viewID, String activity,
			int pathNodeId, SQLiteDatabase database) throws McdException {
		String data = null;
		// later on add code to even check if the view is of type calendar view
		// etc.
		...
		if (v instanceof EditText || v instanceof SearchView) {
			...
			Cursor cursor = database.query(McdDB.TABLE_PATH_NODE_DATA,
					new String[] { McdDB.COLUMN_DATA_ID,
							McdDB.COLUMN_IS_CUSTOMIZED }, McdDB.COLUMN_NODE_ID
							+ " = ?",
					new String[] { String.valueOf(pathNodeId) }, null, null,
					null);
			if (cursor.moveToFirst()) {
				dataID = cursor.getInt(cursor
						.getColumnIndexOrThrow(McdDB.COLUMN_DATA_ID));
				isCustomized = cursor.getInt(cursor
						.getColumnIndexOrThrow(McdDB.COLUMN_IS_CUSTOMIZED));
			}
			cursor.close();
			cursor = null;
			...
			if (dataID != -1) {
				if (isCustomized == 0) {
					Cursor typeData = database.rawQuery("SELECT "
							+ McdDB.COLUMN_DATA + ", " + McdDB.COLUMN_ID
							+ " FROM " + McdDB.TABLE_INPUT_SPECIFIC_DATA
							+ " WHERE " + McdDB.COLUMN_INPUT_TYPE + " = ? AND "
							+ McdDB.COLUMN_ORDER + " = (SELECT "
							+ McdDB.COLUMN_ORDER + " FROM "
							+ McdDB.TABLE_INPUT_SPECIFIC_DATA + " WHERE "
							+ McdDB.COLUMN_ID + " = ?) + 1", new String[] {
							String.valueOf(getMappedInputType(inputType)),
							String.valueOf(dataID) });
					if (!typeData.moveToFirst()) {
						v = null; // to avoid any window leak
						mcdRaiseException(
								"Android Bug-checker hit an invalid primary key"
										+ " for InputSpecificData table",
								database);
					}
					ContentValues values = new ContentValues();
					values.put(McdDB.COLUMN_DATA_ID, typeData.getInt(typeData
							.getColumnIndexOrThrow(McdDB.COLUMN_ID)));
					database.update(McdDB.TABLE_PATH_NODE_DATA, values,
							McdDB.COLUMN_NODE_ID + " = ?",
							new String[] { String.valueOf(pathNodeId) });

					dataID = typeData.getInt(typeData
							.getColumnIndexOrThrow(McdDB.COLUMN_ID));
					isCustomized = 0;

					values.clear();
					data = typeData.getString(typeData
							.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
					values.put(McdDB.COLUMN_DATA, data);
					values.put(McdDB.COLUMN_NODE_ID, pathNodeId);
					database.insert(McdDB.TABLE_QUICK_NODE_DATA_MAP, null,
							values);
					...
					Cursor fieldData = database
							.rawQuery(
									"SELECT " + McdDB.COLUMN_DATA + ", "
											+ McdDB.COLUMN_ID + ", "
											+ McdDB.COLUMN_EXPLORE_DEFAULT
											+ " FROM "
											+ McdDB.TABLE_FIELD_SPECIFIC_DATA
											+ " WHERE " + McdDB.COLUMN_GUI_ID
											+ " = ? AND "
											+ McdDB.COLUMN_ACTIVITY_NAME
											+ " = ? AND "
											+ McdDB.COLUMN_VIEW_ID
											+ " = ? AND " + McdDB.COLUMN_ORDER
											+ " = (SELECT "
											+ McdDB.COLUMN_ORDER + " FROM "
											+ McdDB.TABLE_FIELD_SPECIFIC_DATA
											+ " WHERE " + McdDB.COLUMN_ID
											+ " = ?) + 1",
									new String[] { String.valueOf(guid),
											activity,
											String.valueOf(tmpViewID),
											String.valueOf(dataID) });
					...
					if (fieldData.moveToFirst()) {
						values = new ContentValues();
						values.put(
								McdDB.COLUMN_DATA_ID,
								fieldData.getInt(fieldData
										.getColumnIndexOrThrow(McdDB.COLUMN_ID)));
						database.update(McdDB.TABLE_PATH_NODE_DATA, values,
								McdDB.COLUMN_NODE_ID + " = ?",
								new String[] { String.valueOf(pathNodeId) });

						dataID = fieldData.getInt(fieldData
								.getColumnIndexOrThrow(McdDB.COLUMN_ID));
						isCustomized = 1;

						data = fieldData.getString(fieldData
								.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
					} else {
						Cursor cus = database.query(
								McdDB.TABLE_FIELD_SPECIFIC_DATA,
								new String[] { McdDB.COLUMN_EXPLORE_DEFAULT },
								McdDB.COLUMN_ID + " = ?",
						...
						if (!(cus
								.getInt(cus
										.getColumnIndexOrThrow(McdDB.COLUMN_EXPLORE_DEFAULT)) == 1)) {
							v = null; // to avoid any window leak
							mcdRaiseException(
									"Android Bug-checker hit aproblem for "
											+ " FieldSpecificData table",
									database);
						}
						cus.close();
						cus = null;

						Cursor typeData = database
								.query(McdDB.TABLE_INPUT_SPECIFIC_DATA,
										new String[] { McdDB.COLUMN_DATA,
												McdDB.COLUMN_ID },
										McdDB.COLUMN_INPUT_TYPE + " = ? AND "
												+ McdDB.COLUMN_ORDER + " = ?",
										new String[] {
												String.valueOf(getMappedInputType(inputType)),
						...
						if (!(typeData.moveToFirst())) {
							v = null; // to avoid any window leak
							mcdRaiseException(
									"Android Bug-checker hit a problem for"
				...
				Cursor fieldData = database.query(
						McdDB.TABLE_FIELD_SPECIFIC_DATA,
						new String[] { McdDB.COLUMN_DATA, McdDB.COLUMN_ID,
								McdDB.COLUMN_EXPLORE_DEFAULT },
						McdDB.COLUMN_GUI_ID + " = ? AND "
								+ McdDB.COLUMN_ACTIVITY_NAME + " = ? AND "
								+ McdDB.COLUMN_VIEW_ID + " = ? AND "
								+ McdDB.COLUMN_ORDER + " = ?",
						new String[] { String.valueOf(guid), activity,
								String.valueOf(tmpViewID), String.valueOf(0) },
						null, null, null);
				...
				if (fieldData.moveToFirst()) {
					fieldString = fieldData.getString(fieldData
							.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
					fieldDataID = fieldData.getInt(fieldData
							.getColumnIndexOrThrow(McdDB.COLUMN_ID));
					exploreDef = fieldData
							.getInt(fieldData
									.getColumnIndexOrThrow(McdDB.COLUMN_EXPLORE_DEFAULT));
				}
				fieldData.close();
				fieldData = null;
				...
				if (v instanceof SearchView) {
					if (!(((SearchView) v).getQuery() == null || ((SearchView) v)
							.getQuery().toString().equals(""))) {
						defaultText = ((SearchView) v).getQuery().toString();
					}
				} else if (v instanceof EditText) {
					if (!(((EditText) v).getText() == null || ((EditText) v)
				...
				if (fieldDataID != -1) {
					...
					if (defaultText != null) {
						...
						if (!tmpRes.moveToFirst()) {
							Cursor order = database.rawQuery("SELECT MAX("
									+ McdDB.COLUMN_ORDER + ") FROM "
									+ McdDB.TABLE_FIELD_SPECIFIC_DATA
									+ " WHERE " + McdDB.COLUMN_GUI_ID
									+ " = ? AND " + McdDB.COLUMN_ACTIVITY_NAME
									+ " = ?",
									new String[] { String.valueOf(guid),
											activity });
							if (!(order.moveToFirst())) {
								v = null; // to avoid any window leak
								mcdRaiseException(
										"Android Bug-checker expected result for"
												+ " FieldSpecificData table but was not present",
										database);
							}
							values = new ContentValues();
							...
							database.insert(McdDB.TABLE_FIELD_SPECIFIC_DATA,
									null, values);
					...
					if (defaultText != null) {
						...
						int inserID = (int) database.insert(
								McdDB.TABLE_FIELD_SPECIFIC_DATA, null, values);
						...
						values.put(McdDB.COLUMN_IS_CUSTOMIZED, 1);
						database.insert(McdDB.TABLE_PATH_NODE_DATA, null,
								values);
						...
						if (!(typeData.moveToFirst())) {
							...
							mcdRaiseException(
									"Android Bug-checker expected result for "
											+ "InputSpecificData table which was not present",
									database);
						}
						data = typeData.getString(typeData
								.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
				...
				values = new ContentValues();
				database.delete(McdDB.TABLE_QUICK_NODE_DATA_MAP,
						McdDB.COLUMN_NODE_ID + " = ?",
						new String[] { String.valueOf(pathNodeId) });
				values.put(McdDB.COLUMN_DATA, data);
				values.put(McdDB.COLUMN_NODE_ID, pathNodeId);
			...
			// dataID and isCustomized fields have values corresponding to the
			// current updates

			if (isCustomized == 0) {
				Cursor typeData = database.rawQuery(
						"SELECT " + McdDB.COLUMN_ID + " FROM "
								+ McdDB.TABLE_INPUT_SPECIFIC_DATA + " WHERE "
								+ McdDB.COLUMN_INPUT_TYPE + " = ? AND "
								+ McdDB.COLUMN_ORDER + " = (SELECT "
								+ McdDB.COLUMN_ORDER + " FROM "
				...
				if (typeData.getCount() == 0) {
					...
					database.delete(McdDB.TABLE_PATH_NODE_DATA,
							McdDB.COLUMN_NODE_ID + " = ?",
							new String[] { String.valueOf(pathNodeId) });
					...
					database.execSQL("DELETE FROM "
							...
							+ McdDB.COLUMN_NODE_ID + " = "
							+ String.valueOf(pathNodeId) + " AND "
							+ McdDB.COLUMN_EVENT_ID + " IN (SELECT "
							+ McdDB.TABLE_PATH_NODE + "."
							+ McdDB.COLUMN_EVENT_ID + " FROM "
							+ McdDB.TABLE_PATH_NODE + " WHERE "
							+ McdDB.TABLE_PATH_NODE + "." + McdDB.COLUMN_ID
				...
				Cursor fieldData = database.rawQuery(
						"SELECT " + McdDB.COLUMN_ID + ", "
								+ McdDB.COLUMN_EXPLORE_DEFAULT + " FROM "
								+ McdDB.TABLE_FIELD_SPECIFIC_DATA + " WHERE "
								+ McdDB.COLUMN_GUI_ID + " = ? AND "
								+ McdDB.COLUMN_ACTIVITY_NAME + " = ? AND "
								+ McdDB.COLUMN_VIEW_ID + " = ? AND "
								+ McdDB.COLUMN_ORDER + " = (SELECT "
	...
	public void clickDown(View v) {
		...
		if (v.getParent() instanceof AbsListView
	...
	public void triggerUIEvent(final View v, final int eventType,
			final String data) throws McdException {
		Runnable nextEvent = new Runnable() {
			public void run() {
				// abcDelayExecution(1500);
				Log.e("ABC", "triggered event " + eventType + "on view "
						+ (v != null ? v.toString() : "key-press"));
				// log trigger-event for race detection
				if (v != null
						&& (v.getParent() instanceof AbsListView || v
								.getParent() instanceof AbsSpinner)) {
				...
				if (eventType == EVENT_BACK) {
					...
					getVisibleActivity().dispatchKeyEvent(
					...
					Thread.currentThread().abcEnableLifecycleEvent(
							...
							getVisibleActivity().hashCode(),
							AbcGlobal.ABC_PAUSE);
					AbcGlobal.isPrevEventBackPress = true;

					// log this to a file later
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: BACK press event");
					}
				}
				// menu click does not work programmatically like BACK button
				// click as of now
					...
					if (mWindowManager.getDefaultDisplay().getRotation() == Surface.ROTATION_0)
						getVisibleActivity().setRequestedOrientation(
								ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
					else if (mWindowManager.getDefaultDisplay().getRotation() == Surface.ROTATION_90)
						getVisibleActivity().setRequestedOrientation(
								ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);

					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SCREEN ROTATE event");
					}
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: CLICK event on view " + v.getClass()
										+ " viewID:" + v.getId());
					}
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: LONG CLICK event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
				} else if (eventType == EVENT_SET_TEXT) {
					if (!EditText.class.isInstance(v)) {
						try {
							throw new McdException(
									"Android Bug-checker hit a view expected to be"
					...
					// this may lead to problems in case clicking on editText
					// takes you to different screen
					// but there is a deadlock bug in requestFocus. clicking and
					// typing is a natural thing to do though
					clickDown(v);
					clickUp(v);
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET TEXT event on view " + v.getClass()
										+ " viewID:" + v.getId() + " text: "
										+ data);
					}
					...
					if (!(((CalendarView) v).getMaxDate() >= ((CalendarView) v)
							.getMinDate())) {
						try {
							throw new McdException(
									"Android Bug-checker found a CalendarView with "
					...
					((CalendarView) v)
							.setDate(
									(((CalendarView) v).getMaxDate() + ((CalendarView) v)
											.getMinDate()) / 2, true, true);

					// log this to a file later
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET DATE event on view " + v.getClass()
										+ " viewID:" + v.getId());
					}
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET CURRENT_HOUR_MIN event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
					...
					// ERROR: report error if foll. assertion fails
					if (((DatePicker) v).getMaxDate() < ((DatePicker) v)
							.getMinDate()) {
						try {
							throw new McdException(
									"Android Bug-checker found a DatePicker with "
					...
					cal.setTimeInMillis((((DatePicker) v).getMaxDate() + ((DatePicker) v)
							.getMinDate()) / 2);
					((DatePicker) v).updateDate(cal.get(Calendar.YEAR),
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: UPDATE DATE event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET PROGRESS event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
					...
					// ERROR: report error if foll. assertion fails
					if (!(((NumberPicker) v).getMaxValue() >= ((NumberPicker) v)
							.getMinValue())) {
						try {
							throw new McdException(
									"Android Bug-checker found a NumberPicker view"
					...
					((NumberPicker) v)
							.setValue((((NumberPicker) v).getMaxValue() + ((NumberPicker) v)
									.getMaxValue()) / 2);

					// log this to a file later
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET VALUE (number) event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
					...
					((RatingBar) v)
							.setRating(((RatingBar) v).getNumStars() / 2);

					// log this to a file later
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: SET RATING event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: TOGGLE event on view " + v.getClass()
										+ " viewID:" + v.getId());
					}
					...
					if (((StackView) v).getCount() > 1) {
						if (((StackView) v).getDisplayedChild() < (((StackView) v)
								.getCount() - 1))
					...
					if (ModelCheckingDriver.MODE == ModelCheckingDriver.PLAY) {
						Log.v(ModelCheckingDriver.TAG,
								"error: STACKVIEW SHOW NEXT event on view "
										+ v.getClass() + " viewID:" + v.getId());
					}
				}
	...
	// returns list of <relativePos,viewType,parentId> of hierarchy leading upto
	// list/spinner not
	// including the list/spinner id itself. this is the result provided the
	// view is inside a
	// list/spinner item
	public List<McdTriple> isViewInsideListOrSpinnerItem(int viewID,
		...
		Cursor cursor = database.query(McdDB.TABLE_VIEW_NODE, new String[] {
				McdDB.COLUMN_PARENT_ID, McdDB.COLUMN_VIEW_TYPE,
				McdDB.COLUMN_RELATIVE_ID }, McdDB.COLUMN_ID + " = ?",
				new String[] { String.valueOf(viewID) }, null, null, null);
		if (!(cursor.moveToFirst())) {
			mcdRaiseException(
					"Android Bug-checker found an invalid key in ViewNode table",
					database);
		}
		int viewType = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE));
		int parentID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_PARENT_ID));
		int relativePos = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_RELATIVE_ID));
		while (!((viewType == VIEW_LIST_VIEW || viewType == VIEW_ABS_LIST_VIEW
				|| viewType == VIEW_SPINNER || viewType == VIEW_ABS_SPINNER || viewType == VIEW_EXPANDABLE_LIST_VIEW) || parentID == -1)) {
			...
			cursor = database
					.query(McdDB.TABLE_VIEW_NODE, new String[] {
							McdDB.COLUMN_PARENT_ID, McdDB.COLUMN_VIEW_TYPE,
							McdDB.COLUMN_RELATIVE_ID }, McdDB.COLUMN_ID
							+ " = ?",
			...
			viewType = cursor.getInt(cursor
					.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE));
			parentID = cursor.getInt(cursor
					.getColumnIndexOrThrow(McdDB.COLUMN_PARENT_ID));
			relativePos = cursor.getInt(cursor
					.getColumnIndexOrThrow(McdDB.COLUMN_RELATIVE_ID));
	...
	public void removeUiEventInHierarchy(List<McdTriple> relativePosLst,
			int uiEvent, int uiEnvID, int nodeID, SQLiteDatabase db)
			throws McdException {
		int lstSize = relativePosLst.size();
		Cursor lstCur = db.query(McdDB.TABLE_VIEW_NODE, new String[] {
				McdDB.COLUMN_VIEW_TYPE, McdDB.COLUMN_ID },
				McdDB.COLUMN_PARENT_ID + " = ?", new String[] { String
						.valueOf(relativePosLst.get(lstSize - 1).third) },
				null, null, null);
		if (!lstCur.moveToFirst()) {
			mcdRaiseException(
					"Android Bug-checker found inconsistency in ViewNode table",
					db);
		}

		for (int i = 0; i < lstCur.getCount(); i++) {
			if (lstCur.getInt(lstCur
					.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)) == relativePosLst
					.get(lstSize - 1).second) {
				Cursor tmp = null;
				int tmpParent = lstCur.getInt(lstCur
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				...
				for (int j = 2; j <= lstSize; j++) {
					tmp = db.query(
							...
							McdDB.COLUMN_PARENT_ID + " = ? AND "
									+ McdDB.COLUMN_RELATIVE_ID + " = ?",
							new String[] {
									String.valueOf(tmpParent),
									String.valueOf(relativePosLst.get(lstSize
											- j).first) }, null, null, null);
					if (tmp.moveToFirst()) {
						tmpParent = tmp.getInt(tmp
								.getColumnIndexOrThrow(McdDB.COLUMN_ID));
						if (tmp.getInt(tmp
								.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)) == relativePosLst
								.get(lstSize - j).second) {
							flagBreak = false;
						} else {
							Log.e(TAG,
									"Android Bug-checker expected same view type "
						...
						Log.e(TAG,
								"Android Bug-checker expected a view inside a list "
					...
					Cursor cur = db
							.query(McdDB.TABLE_UI_EVENT,
									...
									McdDB.COLUMN_VIEW_ID + " = ? AND "
											+ McdDB.COLUMN_UI_EVENT_TYPE
											+ " = ?",
									new String[] { String.valueOf(tmpParent),
											String.valueOf(uiEvent) }, null,
									null, null);
					if (cur.moveToFirst()) {
						ContentValues cv = new ContentValues();
						cv.put(McdDB.COLUMN_EVENT_ID, cur.getInt(cur
								.getColumnIndexOrThrow(McdDB.COLUMN_ID)));
						cv.put(McdDB.COLUMN_EVENT_TYPE, 0);
						cv.put(McdDB.COLUMN_UI_ENV_ID, uiEnvID);
						cv.put(McdDB.COLUMN_NODE_ID, nodeID);
						db.insert(McdDB.TABLE_IGNORE_EVENT, null, cv);
	...
	public View reachAndGetViewFromID(int viewID, int screenID,
			int viewRootLimit, SQLiteDatabase database) throws McdException {
		View targetView = null;
		Cursor cursor = database.query(McdDB.TABLE_VIEW_NODE, new String[] {
				McdDB.COLUMN_RELATIVE_ID, McdDB.COLUMN_PARENT_ID,
				McdDB.COLUMN_GUI_ID, McdDB.COLUMN_VIEW_TYPE }, McdDB.COLUMN_ID
				+ " = ?", new String[] { String.valueOf(viewID) }, null, null,
				null);
		if (!(cursor.moveToFirst())) {
			mcdRaiseException(
					"Android Bug-checker found an invalid primary key"
							+ " for ViewNode table", database);
		}

		int relativeID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_RELATIVE_ID));
		int parentID = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_PARENT_ID));
		int guid = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_GUI_ID));
		int viewType = cursor.getInt(cursor
				.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE));
		...
		if (parentID != -1) {
			View parentView = reachAndGetViewFromID(parentID, screenID,
					viewRootLimit, database);
			if (!((ViewGroup.class.isInstance(parentView) && ((ViewGroup) parentView)
					.getChildCount() > relativeID))) {
				parentView = null; // to avoid any window leak
				mcdRaiseException(
						"Android Bug-checker found a mismatch bug when reaching a View"
								+ " using viewId", database);
			}

			targetView = ((ViewGroup) parentView).getChildAt(relativeID);

			if (!(((getSimplifiedClassOfView(targetView.getClass()) == viewType) && targetView
					.getId() == guid))) {
				parentView = null;
			...
			Cursor tmp = database.query(McdDB.TABLE_VIEW_NODE,
					new String[] { McdDB.COLUMN_POPUP_ID }, McdDB.COLUMN_ID
							+ " = ?", new String[] { String.valueOf(viewID) },
					null, null, null);
			tmp.moveToFirst();
			int popupID = tmp.getInt(tmp
					.getColumnIndexOrThrow(McdDB.COLUMN_POPUP_ID));
			tmp.close();
			tmp = null;
			...
			if (popupID == -1) {
				if (viewRootLimit < 1) {
					mcdRaiseException("view root limit cannot be less than 1",
							database);
				}
				targetView = WindowManagerImpl.getDefault().getViewRoots()[n
						- viewRootLimit].getView();
			} else {
				Cursor viewLevel = database.query(
						McdDB.TABLE_POPUPS_SCREEN,
						new String[] { McdDB.COLUMN_VIEWROOT_LEVEL },
						McdDB.COLUMN_SCREEN_ID + " = ? AND "
								+ McdDB.COLUMN_POPUP_ID + " = ?",
						new String[] { String.valueOf(screenID),
								String.valueOf(popupID) }, null, null, null);

				if (!(viewLevel.moveToFirst())) {
					mcdRaiseException(
							"Android Bug-checker found no corresponding viewRootLevel"
									+ " for corresponding screen and popup window in PopupScreens table",
							database);
				}
				int level = viewLevel.getInt(viewLevel
						.getColumnIndexOrThrow(McdDB.COLUMN_VIEWROOT_LEVEL));
				viewLevel.close();
				viewLevel = null;
	...
	public void backtrack(SQLiteDatabase database, McdDB mcdDb,
			final int errorFlag) {
		if (!abcSilentReturn) {

			this.abcSilentReturn = true;
			// todo: write logic for final exit when you finish all the
			// exploring
			database.execSQL("UPDATE " + McdDB.TABLE_PATH + " SET "
					+ McdDB.COLUMN_EXECUTION_STATUS + " = 1 WHERE "
					+ McdDB.COLUMN_ID + " = (SELECT MIN(" + McdDB.COLUMN_ID
					+ ") FROM " + McdDB.TABLE_PATH + ");");

			// node to be backtracked to...
			database.execSQL("UPDATE " + McdDB.TABLE_PATH_NODE + " SET "
					+ McdDB.COLUMN_EVENT_TYPE + " = -1 WHERE "
					+ McdDB.TABLE_PATH_NODE + "." + McdDB.COLUMN_ID
					+ " IN (SELECT " + McdDB.COLUMN_NODE_ID + " FROM "
					+ McdDB.TABLE_PATH + " WHERE " + McdDB.TABLE_PATH + "."
					+ McdDB.COLUMN_ID + " = (SELECT MAX(" + McdDB.TABLE_PATH
					+ "." + McdDB.COLUMN_ID + ") FROM " + McdDB.TABLE_PATH
					+ "));");

			// remove ignore events added at this node
			database.execSQL("DELETE FROM " + McdDB.TABLE_IGNORE_EVENT
					+ " WHERE " + McdDB.TABLE_IGNORE_EVENT + "."
					+ McdDB.COLUMN_NODE_ID + " IN (SELECT " + McdDB.TABLE_PATH
					+ "." + McdDB.COLUMN_NODE_ID + " FROM " + McdDB.TABLE_PATH
					+ " WHERE " + McdDB.TABLE_PATH + "." + McdDB.COLUMN_ID
					+ " = (SELECT MAX(" + McdDB.TABLE_PATH + "."
					+ McdDB.COLUMN_ID + ") FROM " + McdDB.TABLE_PATH + "));");

			database.execSQL("DELETE FROM " + McdDB.TABLE_QUICK_NODE_DATA_MAP
					+ " WHERE " + McdDB.TABLE_QUICK_NODE_DATA_MAP + "."
					+ McdDB.COLUMN_NODE_ID + " IN (SELECT " + McdDB.TABLE_PATH
					+ "." + McdDB.COLUMN_NODE_ID + " FROM " + McdDB.TABLE_PATH
					+ " WHERE " + McdDB.TABLE_PATH + "." + McdDB.COLUMN_ID
					+ " = (SELECT MAX(" + McdDB.TABLE_PATH + "."
			...
			// code to send instructions to server to backup model-checking DB,
			// kill this app,
			...
			// worker.start();
			// contacting local server willnot work if the app does not have
			// INTERNET permission
			// this is done by contacting our own app which has necessary perms
			// and it informs the server
			...
			if (success == 1) {
				...
				multiRaceCount = Thread.currentThread()
						.abcGetMultiThreadedRaceCount();
				asyncRaceCount = Thread.currentThread().abcGetAsyncRaceCount();
				delayPostRaceCount = Thread.currentThread()
						.abcGetDelayPostRaceCount();
				crossPostRaceCount = Thread.currentThread()
						.abcGetCrossPostRaceCount();
				uiRacecount = Thread.currentThread()
						.abcGetCoEnabledEventUiRaces();
				nonUiRaceCount = Thread.currentThread()
						.abcGetCoEnabledEventNonUiRaces();
			}

			Log.e("ABC", "traceGenTime:" + traceGenerationTime
					+ "  raceDetectTime:" + raceDetectionTime);
			...
			intent.putExtra("traceGenerationTime",
					TimeUnit.MILLISECONDS.toSeconds(traceGenerationTime));
			intent.putExtra("raceDetectionTime",
					TimeUnit.MILLISECONDS.toSeconds(raceDetectionTime));
			...
			/*
			 * if(errorFlag != FLAG_ERROR){ long count = Long.MAX_VALUE;
			 * while(count > Long.MIN_VALUE){ count = count-2; count++; } }
			 */
		} else {
			try {
				if (database != null && database.isOpen()) {
	...
	public void backtrackOnError() throws McdException {
		if (!abcSilentReturn) {
			...
			Cursor cursor = database.query(McdDB.TABLE_PATH,
					new String[] { McdDB.COLUMN_NODE_ID }, null, null, null,
					null, McdDB.COLUMN_ID + " DESC");
			int nextEvent = -1;

			// storing the error trace into TABLE_ERROR_PATH,
			// the last node in path has nextEvent = -1 and first node has
			// isFirstNode = 1
			if (cursor.moveToFirst()) {
				do {
					Cursor nodeRes = database
							.query(McdDB.TABLE_PATH_NODE,
									new String[] { McdDB.COLUMN_EVENT_ID,
											McdDB.COLUMN_EVENT_TYPE },
									McdDB.COLUMN_ID + " = ?",
									new String[] { String.valueOf(cursor.getInt(cursor
											.getColumnIndexOrThrow(McdDB.COLUMN_NODE_ID))) },
									null, null, null);

					if (!nodeRes.moveToFirst()) {
						mcdRaiseException(
								"Android bug-checker found an invalid key in PathNode table",
								database);
					}

					String data = null;
					if (nodeRes.getInt(nodeRes
							.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE)) == 0) {
						Cursor dataRes = database
								.query(McdDB.TABLE_QUICK_NODE_DATA_MAP,
										new String[] { McdDB.COLUMN_DATA },
										McdDB.COLUMN_NODE_ID + " = ?",
										new String[] { String.valueOf(cursor.getInt(cursor
												.getColumnIndexOrThrow(McdDB.COLUMN_NODE_ID))) },
						...
						if (dataRes.moveToFirst()) {
							data = dataRes.getString(dataRes
									.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
						}
						dataRes.close();
					}
				...
				if (nextEvent != -1) {
					...
					database.update(McdDB.TABLE_ERROR_PATH, value,
							McdDB.COLUMN_ID + " = ?",
							new String[] { String.valueOf(nextEvent) });
				}
			}
			cursor.close();
	...
	// note: this method arguments have to be modified when you want to express
	// apps with no UI
	// returns int array of size 2. [0]:eventID, [1]:eventType
	public int[] selectNextEventToTrigger(int pathNodeID, int ui_envID,
			SQLiteDatabase database) throws McdException {
		int[] eventInfo = { -1, -1 };

		// if event selected is of the type text input, then do not remove it
		// here from
		// unexplored list, this is handled in retrieveNextTextForView

		// in select event always get the highest <viewType,event> available for
		// the pathNodeID in unexploredList.

		// Select the Intents and UI-events with probability 0.25 and 0.75
		// respectively
	...
	public boolean isViewRootFocused(int n) {
		...
		focused = WindowManagerImpl.getDefault().getViewRoots()[n].getView()
				.hasWindowFocus();
		return focused;
	}

	/*
	 * public void setMenuOnExistingActivityIntance(int activityHash, String
	 * activityName){ McdDB mcdDB = new McdDB(Looper.mcd.getContext());
	 * SQLiteDatabase database = mcdDB.getReadableDatabase(); Cursor cursor =
	 * database.query(McdDB.TABLE_ACTIVITY, new String[]{
	 * McdDB.COLUMN_HAS_MENU}, McdDB.COLUMN_ACTIVITY_NAME + " = ? AND " +
	 * McdDB.COLUMN_TMP_ACTIVITY_HASH + " = ?", new String[]{activityName,
	 * String.valueOf(activityHash)}, null, null, null);
	 * if(cursor.moveToFirst()){ hasMenu =
	 * (cursor.getInt(cursor.getColumnIndexOrThrow( McdDB.COLUMN_HAS_MENU)) ==
	 * 1)? true : false; } cursor.close(); database.close(); cursor = null;
	 * database = null; }
	 */
	...
	public int getViewRootInFocus() {
		...
		for (int i = 1; i <= WindowManagerImpl.getDefault().getViewRoots().length; i++) {
			if (isViewRootFocused(WindowManagerImpl.getDefault().getViewRoots().length
					- i)) {
				viewRootInFocus = WindowManagerImpl.getDefault().getViewRoots().length
						- i;
				break;
			}
		}
	...
	public void mcdRaiseException(String msg, SQLiteDatabase db)
			throws McdException {
		try {
		...
		if (abcSilentReturn) {
			...
			if (getVisibleActivity().hashCode() != previousEventActivity) {
				if (isCorrectViewOnTop == false) {
					...
					// maybe only this check is sufficient and
					// isCorrectViewOnTop is not needed,
			...
			if (MODE == EXPLORE) {
				pathRes = database.query(McdDB.TABLE_PATH, new String[] {
						McdDB.COLUMN_ID, McdDB.COLUMN_NODE_ID },
						McdDB.COLUMN_EXECUTION_STATUS + " = ?",
						new String[] { String.valueOf(1) }, null, null, null);

				if (!(pathRes.getCount() == 1 || pathRes.getCount() == 0)) {
					mcdRaiseException(
							"Android Bug-checker expected PATH table to return"
									+ " only one or no row, which was not the case",
							database);
				}
			} else if (MODE == PLAY) {
				if (playNextEvent == -1) {
					pathRes = database.rawQuery("SELECT " + McdDB.COLUMN_ID
							+ ", " + McdDB.COLUMN_EVENT_ID + ", "
							+ McdDB.COLUMN_EVENT_TYPE + ", "
							+ McdDB.COLUMN_DATA + ", "
							+ McdDB.COLUMN_NEXT_EVENT + " FROM "
							+ McdDB.TABLE_ERROR_PATH + " WHERE "
							+ McdDB.COLUMN_IS_FIRST_NODE + " = ? "
							+ " ORDER BY " + McdDB.COLUMN_ID + " ASC"
							+ " LIMIT 1", new String[] { String.valueOf(1) });
				} else {
					pathRes = database.query(McdDB.TABLE_ERROR_PATH,
							new String[] { McdDB.COLUMN_ID,
									McdDB.COLUMN_EVENT_ID,
									McdDB.COLUMN_EVENT_TYPE, McdDB.COLUMN_DATA,
									McdDB.COLUMN_NEXT_EVENT }, McdDB.COLUMN_ID
									+ " = ?",
							new String[] { String.valueOf(playNextEvent) },
							null, null, null);
				}
			}
			...
			if (pathRes.moveToFirst() && MODE == EXPLORE) {
				pathID = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				nodeToExecute = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_NODE_ID));
			} else if (pathRes.moveToFirst() && MODE == PLAY) {
				playEventID = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_ID));
				playEventType = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE));
				playData = pathRes.getString(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_DATA));
				playNextEvent = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_NEXT_EVENT));
				pathID = pathRes.getInt(pathRes
						.getColumnIndexOrThrow(McdDB.COLUMN_ID));

				// delete this row entry in error table. Always maintain a copy
				// of database before replay!
				// it is deleted just for the coding logic, the same table
				// stores many error paths..
				// each complete run of a play deletes a eror path stored, so
				// that in nextplay run
				// another error path is re-played! Its better if this logic is
				// modified to store error
			...
			if (MODE == PLAY && playEventID != -1) {
				if (playEventType == UI_EVENT) {
					Cursor res = database.query(McdDB.TABLE_UI_EVENT,
							new String[] { McdDB.COLUMN_VIEW_ID,
									McdDB.COLUMN_UI_EVENT_TYPE },
							McdDB.COLUMN_ID + " = ?",
					...
					int viewID = res.getInt(res
							.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_ID));
					Cursor screen = database.query(McdDB.TABLE_VIEW_NODE,
							new String[] { McdDB.COLUMN_SCREEN_ID },
							McdDB.COLUMN_ID + " = ?",
							new String[] { String.valueOf(viewID) }, null,
							null, null);
					...
					int uiEventType = res.getInt(res
							.getColumnIndexOrThrow(McdDB.COLUMN_UI_EVENT_TYPE));
					int screenID = screen.getInt(screen
							.getColumnIndexOrThrow(McdDB.COLUMN_SCREEN_ID));
					...
					int viewRootLimit = -1;
					ViewRootImpl[] tmpViewRoots = WindowManagerImpl
							.getDefault().getViewRoots();

					// modified logic to handle popup windows which ignores
					// popup windows whose base is not a viewGroup
					...
					// if no viewGroup view in popup windows then trigger event
					// on the activity screen below all poup windows
					if (viewGrpPopupsExist == false && popupWindows.size() > 0) {
						int viewInFocus = getViewRootInFocus();
						if (viewInFocus == -1) {
							tmpViewRoots = null; // to avoid any window leak
							mcdRaiseException(
									"re-execution not reaching same UI sate. We have "
						...
						viewRootLimit = tmpViewRoots.length
								- getViewRootInFocus();
					}
					...
					if (uiEventType != EVENT_BACK
							&& uiEventType != EVENT_MENU_CLICK
							&& uiEventType != EVENT_ROTATE_SCREEN) {
						tmpView = reachAndGetViewFromID(viewID, screenID,
								viewRootLimit, database);
					}

					// adding to path is not needed as the node is already in
					// path. only updating
					// the node with event info was needed which has already
					// been done
				...
				if (abcHangCounter < HANG_LIMIT) {
					Cursor tmpNode = database.rawQuery("SELECT "
							+ McdDB.COLUMN_EVENT_ID + ", "
							+ McdDB.COLUMN_EVENT_TYPE + " FROM "
							+ McdDB.TABLE_PATH_NODE + " WHERE "
							+ McdDB.COLUMN_ID + " IN (SELECT "
							+ McdDB.TABLE_PATH_NODE + "." + McdDB.COLUMN_ID
							+ " FROM " + McdDB.TABLE_PATH_NODE + " WHERE "
							+ McdDB.TABLE_PATH_NODE + "." + McdDB.COLUMN_ID
							+ " < " + nodeToExecute + " ORDER BY "
							+ McdDB.TABLE_PATH_NODE + "." + McdDB.COLUMN_ID
							+ " DESC LIMIT 1);", null);
					if (tmpNode.moveToFirst()) {
						// eventType = 0 for Ui event and eventType = 1 for
						// intent
						if (tmpNode
								.getInt(tmpNode
										.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE)) == UI_EVENT) {
							Cursor tmpUiEvent = database
									.query(McdDB.TABLE_UI_EVENT,
											new String[] { McdDB.COLUMN_UI_EVENT_TYPE },
											McdDB.COLUMN_ID + " = ?",
											new String[] { String.valueOf(tmpNode.getInt(tmpNode
													.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_ID))) },
											null, null, null);
							...
							if (tmpUiEvent
									.getInt(tmpUiEvent
											.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE)) == EVENT_CLICK) {
								if (viewClickDone == false) {
									tmpUiEvent.close();
									tmpNode.close();
									try {
									...
									.getInt(tmpUiEvent
											.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE)) == EVENT_LONG_CLICK) {
								if (viewLongClickDone == false) {
									tmpUiEvent.close();
									tmpNode.close();
									try {
				...
				Cursor nodeResult = database
						.query(McdDB.TABLE_PATH_NODE, new String[] {
								McdDB.COLUMN_EVENT_ID, McdDB.COLUMN_EVENT_TYPE,
								McdDB.COLUMN_UI_ENV_ID }, McdDB.COLUMN_ID
								+ " = ?",
				...
				if (!(nodeResult.getCount() == 1)) {
					mcdRaiseException(
							"Android Bug-checker expected PathNode table to"
									+ " return only one row as result which was not the case",
							database);
				} else {
					nodeResult.moveToFirst();
					uiEnvID = nodeResult.getInt(nodeResult
							.getColumnIndexOrThrow(McdDB.COLUMN_UI_ENV_ID));
					eventID = nodeResult.getInt(nodeResult
							.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_ID));
					eventType = nodeResult.getInt(nodeResult
							.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE));
				}
				nodeResult.close();
				nodeResult = null;

				// code to set execution status of next node in path table to 1,
				// if no next
				// node then do nothing
				ContentValues cv = new ContentValues();
				cv.put(McdDB.COLUMN_EXECUTION_STATUS, -1);
				database.update(McdDB.TABLE_PATH, cv, McdDB.COLUMN_ID + " = ?",
						new String[] { String.valueOf(pathID) });

				database.execSQL("UPDATE " + McdDB.TABLE_PATH + " SET "
						+ McdDB.COLUMN_EXECUTION_STATUS + " = 1 WHERE "
						+ McdDB.TABLE_PATH + "." + McdDB.COLUMN_NODE_ID
						+ " IN (SELECT " + McdDB.TABLE_PATH_NODE + "."
						+ McdDB.COLUMN_ID + " FROM " + McdDB.TABLE_PATH_NODE
						+ " WHERE " + McdDB.TABLE_PATH_NODE + "."
						+ McdDB.COLUMN_ID + " > " + nodeToExecute
						+ " ORDER BY " + McdDB.TABLE_PATH_NODE + "."
						+ McdDB.COLUMN_ID + " ASC LIMIT 1);");

				// this query returns activity name corresponding to activityID
				// obtained from ui_envID,
				// provided activityID != -1. Update temporary activity hash
				// only if a result is returned
				Cursor activityInfo = database.rawQuery("SELECT "
						+ McdDB.COLUMN_ACTIVITY_NAME + ", "
						+ McdDB.TABLE_ACTIVITY + "." + McdDB.COLUMN_ID
						+ " FROM " + McdDB.TABLE_ACTIVITY + " JOIN "
						+ McdDB.TABLE_UI_ENV + " ON " + McdDB.TABLE_ACTIVITY
						+ "." + McdDB.COLUMN_ID + " = " + McdDB.TABLE_UI_ENV
						+ "." + McdDB.COLUMN_ACTIVITY_ID + " WHERE "
						+ McdDB.TABLE_UI_ENV + "." + McdDB.COLUMN_ID + " = ?",
						new String[] { String.valueOf(uiEnvID) });

				int viewRootLimit = -1;

				if (activityInfo.moveToFirst()) {
					String activityNameTmp = activityInfo
							.getString(activityInfo
									.getColumnIndexOrThrow(McdDB.COLUMN_ACTIVITY_NAME));
					String curActivityName = getVisibleActivity()
							.getLocalClassName();

					if (!(activityInfo.getCount() == 1 && getVisibleActivity()
							.getLocalClassName()
							.equals(activityInfo.getString(activityInfo
									.getColumnIndexOrThrow(McdDB.COLUMN_ACTIVITY_NAME))))) {
						mcdRaiseException(
								"Android Bug-checker found some mismatch in Activity table",
								database);
					}
					ContentValues tmpVal = new ContentValues();
					tmpVal.put(McdDB.COLUMN_TMP_ACTIVITY_HASH,
							getVisibleActivity().hashCode());
					database.update(
							McdDB.TABLE_ACTIVITY,
							tmpVal,
							McdDB.COLUMN_ID + " = ?",
							new String[] { String.valueOf(activityInfo.getInt(activityInfo
									.getColumnIndexOrThrow(McdDB.COLUMN_ID))) });
					...
					// modified logic to handle popup windows which ignores
					// popup windows whose base is not a viewFroup
					boolean viewGrpPopupsExist = false;
					...
					if (popupsCreatedOnLastEvent > 0) {
						for (int l = 1; l <= popupsCreatedOnLastEvent; l++) {
							if (tmpViewRoots[tmpViewRoots.length - l].getView() instanceof ViewGroup) {
								viewGrpPopupsExist = true;
								ViewGroup vg = (ViewGroup) tmpViewRoots[tmpViewRoots.length
										- l].getView();
								tmpScreenHash += computeScreenHash(vg,
										tmpScreenHash);
							}
						}
					}
					...
					// on the activity screen below all poup windows
					if (viewGrpPopupsExist == false && popupWindows.size() > 0) {
						// if(!(isViewRootFocused(tmpViewRoots.length -
						// (popupWindows.size() + 1))))
						int viewInFocus = getViewRootInFocus();
						if (viewInFocus == -1) {
							tmpViewRoots = null;
					...
					if (viewGrpPopupsExist == false) {
						tmpScreenHash = computeScreenHash(
								(ViewGroup) tmpViewRoots[tmpViewRoots.length
										- viewRootLimit].getView(), 0);
					}
				...
				if (eventType == -1) {
					// control has reached the backtrack node, select a new
					// event and explore

					int[] nextEvent = selectNextEventToTrigger(nodeToExecute,
							uiEnvID, database);
					updatePathNodeWithEvent(nodeToExecute, nextEvent[0],
							nextEvent[1], database);
					if (nextEvent[1] == -1) {
						// no event to be triggered at this point. so backtrack
						// delete any event remaining in unexplored loist but
						// not triggered due to their
						...
						database.delete(McdDB.TABLE_PATH, McdDB.COLUMN_NODE_ID
								+ " = ?",
								new String[] { String.valueOf(nodeToExecute) });
						database.delete(McdDB.TABLE_FIELD_SPECIFIC_DATA,
								McdDB.COLUMN_NODE_ID + " = ?",
								new String[] { String.valueOf(nodeToExecute) }); // to
																					// remove
																					// any
																					// default
																					// text
						...
						Cursor btCursor = database.rawQuery(
								"SELECT COUNT(*) FROM " + McdDB.TABLE_PATH,
								null);
					...
					} else if (nextEvent[1] == UI_EVENT) { // eventType is
															// UIevent
						...
						Cursor res = database.query(McdDB.TABLE_UI_EVENT,
								...
								null, null, null);
						Cursor screen = database.query(McdDB.TABLE_UI_ENV,
								new String[] { McdDB.COLUMN_SCREEN_ID },
								McdDB.COLUMN_ID + " = ?",
								new String[] { String.valueOf(uiEnvID) }, null,
								null, null);
						...
						if (uiEventType != EVENT_BACK
							...
							if (textDataViewSet.contains(uiEventType)) {
								textData = retrieveNextDataForView(tmpView,
										viewID, getVisibleActivity()
												.getLocalClassName(),
										nodeToExecute, database);
							}
						}

						// adding to path is not needed as the node is already
						// in path. only updating
						// the node with event info was needed which has already
						// been done
						// addToPath(nodeToExecute, -1, database);
						popupsCreatedOnLastEvent = 0;
					...
					Cursor viewRes = database.query(McdDB.TABLE_UI_EVENT,
							new String[] { McdDB.COLUMN_VIEW_ID,
									McdDB.COLUMN_UI_EVENT_TYPE },
							McdDB.COLUMN_ID + " = ?",
					...
					if (!(viewRes.getCount() == 1)) {
						mcdRaiseException(
								"Android Bug-checker expected UiEvent table to"
										+ " return a single row result which was not the case",
								database);
					}
					viewRes.moveToFirst();
					View targetView = null;
					String data = null;
					int tmpUiEventType = viewRes.getInt(viewRes
							.getColumnIndexOrThrow(McdDB.COLUMN_UI_EVENT_TYPE));

					if (tmpUiEventType != EVENT_BACK
							&& tmpUiEventType != EVENT_MENU_CLICK
							&& tmpUiEventType != EVENT_ROTATE_SCREEN) {
						targetView = reachAndGetViewFromID(
								viewRes.getInt(viewRes
										.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_ID)),
								screen.getInt(screen
										.getColumnIndexOrThrow(McdDB.COLUMN_SCREEN_ID)),
								viewRootLimit, database);
						// get any text data corresponding to the path node
						data = getTextDataStoredForPathNode(nodeToExecute,
								database);
					}
			...
			} else if (MODE == EXPLORE) { // code to scan current app-state and
											// trigger the next event

				// check if previous event was on data widget and apply
				// corresponding logic
				Cursor tmpNodeRes = database.rawQuery("SELECT "
						+ McdDB.COLUMN_NODE_ID + " FROM " + McdDB.TABLE_PATH
						+ " WHERE " + McdDB.COLUMN_ID + " = (SELECT MAX("
						+ McdDB.COLUMN_ID + ") FROM " + McdDB.TABLE_PATH + ")",
						null);

				int prevNodeID = -1;
				if (tmpNodeRes.moveToFirst()) {
					prevNodeID = tmpNodeRes.getInt(tmpNodeRes
							.getColumnIndexOrThrow(McdDB.COLUMN_NODE_ID));
				}
				tmpNodeRes.close();
				tmpNodeRes = null;
				...
				ViewRootImpl[] viewRoots = WindowManagerImpl.getDefault()
						.getViewRoots();
				...
				if (popupsCreatedOnLastEvent > 0) {
					for (int l = 1; l <= popupsCreatedOnLastEvent; l++) {
						if (viewRoots[viewRoots.length - l].getView() instanceof ViewGroup) {
							viewGrpPopupsExist = true;
							ViewGroup vg = (ViewGroup) viewRoots[viewRoots.length
									- l].getView();
				...
				// if no viewGroup view in popup windows then trigger event on
				// the activity screen below all
				...
				if (viewGrpPopupsExist == false && popupWindows.size() > 0) {
					...
					if (viewInFocus != -1)
						...
						viewRootLimit = -2; // perform backtrack because no
											// focused viewRoot
				} else {
					// if(!isPreviousMenuClickEvent)
					// viewRootLimit = 1;
				...
				if (viewGrpPopupsExist == false && viewRootLimit != -2) {
					screenHash = computeScreenHash(
							(ViewGroup) viewRoots[viewRoots.length
									- viewRootLimit].getView(), 0);
				}
				...
				if (prevNodeID != -1) {
					Cursor nodeRes = database.query(McdDB.TABLE_PATH_NODE,
							new String[] { McdDB.COLUMN_EVENT_ID,
									McdDB.COLUMN_EVENT_TYPE,
									McdDB.COLUMN_UI_ENV_ID }, McdDB.COLUMN_ID
									+ " = ?",
							new String[] { String.valueOf(prevNodeID) }, null,
							null, null);

					if (!(nodeRes.moveToFirst())) {
						mcdRaiseException(
								"Android Bug-checker found an invalid key in the "
										+ "PathNode table", database);
					}

					// events like BACK button click, screen rotation etc. will
					// be classified as UI
					// events with viewID = -1 and some uiEventType
					prevEventID = nodeRes.getInt(nodeRes
							.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_ID));
					prevEventType = nodeRes.getInt(nodeRes
							.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE));
					prevUIEnvID = nodeRes.getInt(nodeRes
							.getColumnIndexOrThrow(McdDB.COLUMN_UI_ENV_ID));
					nodeRes.close();
					nodeRes = null;
					...
					if (prevEventType == UI_EVENT) { // ui-event
						...
						viewRes = database.query(McdDB.TABLE_UI_EVENT,
								new String[] { McdDB.COLUMN_VIEW_ID,
										McdDB.COLUMN_UI_EVENT_TYPE },
								McdDB.COLUMN_ID + " = ?",
						...
						// if and not assert because orientation change is also
						// a UI event with view=-1
						if (viewRes.moveToFirst()) {
							prevViewID = viewRes
									.getInt(viewRes
											.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_ID));
							prevUiEventType = viewRes
									.getInt(viewRes
											.getColumnIndexOrThrow(McdDB.COLUMN_UI_EVENT_TYPE));
						}
						viewRes.close();
						...
						Cursor pathLength = database.rawQuery(
								"SELECT COUNT(*) FROM " + McdDB.TABLE_PATH,
								null);
						...
						Cursor tmpHashRes = database
								.query(McdDB.TABLE_UI_ENV,
										new String[] { McdDB.COLUMN_SCREEN_ACTIVITY_HASH },
										McdDB.COLUMN_ID + " = ?",
										new String[] { String
												.valueOf(prevUIEnvID) }, null,
										null, null);
						if (!tmpHashRes.moveToFirst()) {
							mcdRaiseException(
									"Android Bug-checker found an inconsistent primary key"
											+ " in UIEnv table", database);
						}
						int tmpHashValue = tmpHashRes
								.getInt(tmpHashRes
										.getColumnIndexOrThrow(McdDB.COLUMN_SCREEN_ACTIVITY_HASH));
						tmpHashRes.close();
						tmpHashRes = null;
						...
						Cursor activityInfo = database.rawQuery("SELECT "
								...
								+ McdDB.TABLE_ACTIVITY + "." + McdDB.COLUMN_ID
								+ " FROM " + McdDB.TABLE_ACTIVITY + " JOIN "
								+ McdDB.TABLE_UI_ENV + " ON "
								+ McdDB.TABLE_ACTIVITY + "." + McdDB.COLUMN_ID
								+ " = " + McdDB.TABLE_UI_ENV + "."
								+ McdDB.COLUMN_ACTIVITY_ID + " WHERE "
								+ McdDB.TABLE_UI_ENV + "." + McdDB.COLUMN_ID
								+ " = ?",
								new String[] { String.valueOf(prevUIEnvID) });

						activityInfo.moveToFirst();

						// add UI events to ignore events (events those need not
						// be triggered again and agin in a path.
						// E.g., setting an EditText repeatedly) only if
						// previous activity and current
						// activity are the same
						if (activityInfo
								.getString(
										activityInfo
												.getColumnIndexOrThrow(McdDB.COLUMN_ACTIVITY_NAME))
								.equals(getVisibleActivity()
										.getLocalClassName())) {
							if (prevUiEventType == EVENT_MENU_CLICK
									|| prevUiEventType == EVENT_ROTATE_SCREEN) {
								// just a heuristic to avoid multiple menu
								// clicks or rotate screens on the same activity
								ContentValues cv = new ContentValues();
								...
								if (prevViewID > 0) {
									...
									if ((tmpPopID == -1 && popupWindows.size() == 0)
											|| (tmpPopID != -1 && popupWindows
													.size() > 0)) {
										// logic to ignore events
										if (screenHash == tmpHashValue) {

											List<McdTriple> relativePosLst = isViewInsideListOrSpinnerItem(
													prevViewID, database);
											Cursor lstCur = null;
											if (relativePosLst.size() > 0) {
												switch (tmpType) {
												case VIEW_RADIO_BUTTON:
													// reasoning about immediate
													// parent of given view
													if (relativePosLst.size() >= 2
															&& relativePosLst
																	.get(1).second == VIEW_RADIO_GROUP) {
														lstCur = database
																.query(McdDB.TABLE_VIEW_NODE,
																		new String[] { McdDB.COLUMN_ID },
																		McdDB.COLUMN_PARENT_ID
														...
														if (lstCur
																.moveToFirst()) {
															for (int i = 0; i < lstCur
																	.getCount(); i++) {
																Cursor tmpLst = database
																		.query(McdDB.TABLE_UI_EVENT,
																				new String[] { McdDB.COLUMN_ID },
																				McdDB.COLUMN_VIEW_ID
																						+ " = ? AND "
																...
																if (tmpLst
																		.moveToFirst()) {
																	ContentValues cv = new ContentValues();
																	cv.put(McdDB.COLUMN_EVENT_ID,
																			tmpLst.getInt(tmpLst
																					.getColumnIndexOrThrow(McdDB.COLUMN_ID)));
																	cv.put(McdDB.COLUMN_EVENT_TYPE,
																			UI_EVENT);
																	cv.put(McdDB.COLUMN_UI_ENV_ID,
																			prevUIEnvID);
																	cv.put(McdDB.COLUMN_NODE_ID,
																			prevNodeID);
																	database.insert(
																			McdDB.TABLE_IGNORE_EVENT,
																			null,
																			cv);

																	Log.e("abc",
																			"PREVIOUS EVENT ADDED TO IGNORE LIST");
																}
																lstCur.moveToNext();
																tmpLst.close();
																tmpLst = null;
												...
												if (!lstCur.moveToFirst()) {
													mcdRaiseException(
															"Android Bug-checker found invalid key "
																	+ "in ViewNode table",
															database);
												}
												if (lstCur
														.getInt(lstCur
																.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)) == VIEW_RADIO_GROUP) {
													Cursor tmpCur = database
															.query(McdDB.TABLE_VIEW_NODE,
																	new String[] {
																			McdDB.COLUMN_ID,
																			McdDB.COLUMN_VIEW_TYPE },
													...
													if (tmpCur.moveToFirst()) {
														for (int i = 0; i < tmpCur
																.getCount(); i++) {
															if (tmpCur
																	.getInt(tmpCur
																			.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_TYPE)) == VIEW_RADIO_BUTTON) {
																Cursor tmpCur1 = database
																		.query(McdDB.TABLE_UI_EVENT,
																				new String[] { McdDB.COLUMN_ID },
																				McdDB.COLUMN_VIEW_ID
																						+ " = ? AND "
												...
												// usually long clicks are used
												// to get context menu etc.
												// so no point exploring it
												// multiple times if new screens
												// are'nt reached
												ContentValues cv = new ContentValues();
												cv.put(McdDB.COLUMN_EVENT_ID,
														prevEventID);
											...
											&& popupWindows.size() > 0) {
										// do not repeatedly explore events
										// leading to popup windows
										// todo: later on if you find this
										// missing interesting paths, remove
										// this
										// it will still work fine but will
										// repeatedly explore popup windows
										ContentValues cv = new ContentValues();
										cv.put(McdDB.COLUMN_EVENT_ID,
												prevEventID);
										cv.put(McdDB.COLUMN_EVENT_TYPE,
						...
						// bactrack logic in case path depth limit is reached.
						// Similar code
						// is found in the UI_EVENT part too. This control is
						// used only for
						// backtracking or performing any heuristics
						// corresponding to previous
						// event. Next event selection is done later and not
						// here.
				...
				int screenID = -1;
				int ui_envID = -1;
				Cursor activityResult = database
						.query(McdDB.TABLE_ACTIVITY,
								new String[] { McdDB.COLUMN_ID },
								McdDB.COLUMN_ACTIVITY_NAME + " = ? AND "
								...
								new String[] {
										getVisibleActivity()
												.getLocalClassName(),
										String.valueOf(getVisibleActivity()
												.hashCode()) }, null, null,
								null);
				if (!activityResult.moveToFirst()) {
					ContentValues values = new ContentValues();
					values.put(McdDB.COLUMN_ACTIVITY_NAME, getVisibleActivity()
							.getLocalClassName());
					values.put(McdDB.COLUMN_TMP_ACTIVITY_HASH,
							getVisibleActivity().hashCode());
					activityID = (int) database.insert(McdDB.TABLE_ACTIVITY,
							null, values);
				} else {
					activityID = activityResult.getInt(activityResult
							.getColumnIndexOrThrow(McdDB.COLUMN_ID));
				}
				...
				// store and trigger popupWindow events only if there are any
				// ViewGroup based popupWindows
				if (popupWindows.size() > 0 && viewRootLimit == -1) {
					int viewLevelsSize = WindowManagerImpl.getDefault()
							.getViewRoots().length;
					...
					if (prevEventID == -1) {
						...
						// event that created popup windows is an intent, here
						// event type gives the intent id
						tmpViewID = -1;
						tmpEventType = prevEventID;

					} else if (tmpPopID != -1 && popupsCreatedOnLastEvent == 0) {
						// previous event was on a popup window and no new
						// windows were created and,
						// the popup windows are'nt yet destroyed
						Cursor popupRes = database.query(McdDB.TABLE_POPUP,
								new String[] { McdDB.COLUMN_VIEW_ID,
										McdDB.COLUMN_EVENT_TYPE },
								McdDB.COLUMN_ID + " = ?",
						...
						if (!(popupRes.moveToFirst())) {
							mcdRaiseException(
									"Android Bug-checker found an invalid key in"
											+ " Popup table", database);
						}
						tmpViewID = popupRes.getInt(popupRes
								.getColumnIndexOrThrow(McdDB.COLUMN_VIEW_ID));
						tmpEventType = popupRes
								.getInt(popupRes
										.getColumnIndexOrThrow(McdDB.COLUMN_EVENT_TYPE));
						popupRes.close();
						popupRes = null;
					...
					for (int k = 1; k <= windowSize; k++) {
						if (WindowManagerImpl.getDefault().getViewRoots()[viewLevelsSize
								- k].getView() instanceof ViewGroup) {
							matchingPopups = doIdenticalPopupsExist(tmpViewID,
									tmpEventType, k,
									WindowManagerImpl.getDefault()
											.getViewRoots()[viewLevelsSize - k]
											.getView(), database);

							if (matchingPopups == null) { // no matching popups
															// found
								matchingScreenExists = false;

								ContentValues popValues = new ContentValues();
								popValues.put(McdDB.COLUMN_VIEW_ID, tmpViewID);
								popValues.put(McdDB.COLUMN_EVENT_TYPE,
										tmpEventType);
								popValues.put(McdDB.COLUMN_ACTIVITY_ID,
										activityID);
								tmpPopupID = (int) database.insert(
										McdDB.TABLE_POPUP, null, popValues);
								if (!(tmpPopupID > -1)) {
									mcdRaiseException(
											"Android Bug-checker had a failure when "
													+ "inserting a row into Popup table",
											database);
								}
								...
								storeScreenElements(
										WindowManagerImpl.getDefault()
												.getViewRoots()[viewLevelsSize
												- k].getView(), activityID,
										getVisibleActivity()
												.getLocalClassName(),
										tmpPopupID, 0, database);

							} else if (matchingPopups.second.size() != 0
									&& matchingScreenExists) {
								if (k == 1) {
									matchingScreens = matchingPopups.second;
								} else {
									matchingScreens
											.retainAll(matchingPopups.second);
									if (matchingScreens.size() == 0)
										matchingScreenExists = false;
								}
								matchingPopupIDs[k - 1] = matchingPopups.first
										.intValue();
							} else {
								matchingScreenExists = false;
								matchingPopupIDs[k - 1] = matchingPopups.first
					...
					if (matchingScreenExists) {
						...
						for (Integer tmpScreenID : matchingScreens) {
							...
							Cursor tmp = database.query(
									McdDB.TABLE_POPUPS_SCREEN,
									new String[] { McdDB.COLUMN_POPUP_ID },
						...
						if (!matchFound) {
							...
							values.put(McdDB.COLUMN_ACTIVITY_NAME,
									getVisibleActivity().getLocalClassName());
							values.put(McdDB.COLUMN_SCREEN_HASH, 0);
							screenID = (int) database.insert(
									McdDB.TABLE_SCREEN, null, values);

							if (!(screenID > -1)) {
								mcdRaiseException(
										"Android Bug-checker had a failure in"
												+ " inserting a row into Screen table",
										database);
							}
							...
							values.put(McdDB.COLUMN_SCREEN_ID, screenID);
							values.put(McdDB.COLUMN_ACTIVITY_ID, activityID);
							values.put(McdDB.COLUMN_SCREEN_ACTIVITY_HASH,
									screenHash);
							ui_envID = (int) database.insert(
									McdDB.TABLE_UI_ENV, null, values);

							for (int j = 0; j < windowSize; j++) {
								if (WindowManagerImpl.getDefault()
										.getViewRoots()[viewLevelsSize
										- (j + 1)].getView() instanceof ViewGroup) {
									values = new ContentValues();
									values.put(McdDB.COLUMN_POPUP_ID,
											matchingPopupIDs[j]);
									values.put(McdDB.COLUMN_VIEWROOT_LEVEL,
											j + 1);
									values.put(McdDB.COLUMN_SCREEN_ID, screenID);
									database.insert(McdDB.TABLE_POPUPS_SCREEN,
											null, values);
								}
							}
						} else {
							Cursor ui_envRes = database.query(
									McdDB.TABLE_UI_ENV,
									new String[] { McdDB.COLUMN_ID },
									...
									new String[] { String.valueOf(activityID),
											...
											String.valueOf(screenHash) }, null,
									null, null);
							if (ui_envRes.moveToFirst()) {
								...
								uiEnvTmp.put(McdDB.COLUMN_ACTIVITY_ID,
										activityID);
								uiEnvTmp.put(McdDB.COLUMN_SCREEN_ID, screenID);
								uiEnvTmp.put(McdDB.COLUMN_SCREEN_ACTIVITY_HASH,
										screenHash);
								ui_envID = (int) database.insert(
										McdDB.TABLE_UI_ENV, null, uiEnvTmp);
							}
						}
						...
						values.put(McdDB.COLUMN_ACTIVITY_NAME,
								getVisibleActivity().getLocalClassName());
						values.put(McdDB.COLUMN_SCREEN_HASH, 0);
						screenID = (int) database.insert(McdDB.TABLE_SCREEN,
								null, values);

						if (!(screenID > -1)) {
							mcdRaiseException(
									"Android Bug-checker had a failure"
											+ " inserting a row into Screen table",
									database);
						}
						...
						values.put(McdDB.COLUMN_SCREEN_ACTIVITY_HASH,
								screenHash);
						ui_envID = (int) database.insert(McdDB.TABLE_UI_ENV,
								null, values);
					...
					screenID = findIdenticalActivityScreen(
							WindowManagerImpl.getDefault().getViewRoots()[WindowManagerImpl
									.getDefault().getViewRoots().length
									- viewRootLimit].getView(),
							getVisibleActivity().getLocalClassName(), database);
					if (screenID == -1) {
						int[] res = storeScreenElements(WindowManagerImpl
								.getDefault().getViewRoots()[WindowManagerImpl
								.getDefault().getViewRoots().length
								- viewRootLimit].getView(), activityID,
								getVisibleActivity().getLocalClassName(), -1,
								screenHash, database);
						ui_envID = res[0];
						screenID = res[1];
					} else {
						// check if hash corresponding to this screen exists or
						// else
						// create a new (act,sceen,hash) entry in ui_env table
						Cursor hashRes = database.query(
								McdDB.TABLE_UI_ENV,
								new String[] { McdDB.COLUMN_ID },
								McdDB.COLUMN_ACTIVITY_ID + " = ? AND "
										+ McdDB.COLUMN_SCREEN_ID + " = ? AND "
						...
						if (hashRes.moveToFirst()) {
							ui_envID = hashRes.getInt(hashRes
									.getColumnIndexOrThrow(McdDB.COLUMN_ID));
						} else {
							ContentValues values = new ContentValues();
							values.put(McdDB.COLUMN_ACTIVITY_ID, activityID);
				...
				// code to store events of all viewRoots for current screen
				// after creating a
				// dummy path node.
				...
				if (windowSize > 0 && viewRootLimit == -1) {
					Cursor tmp = database.query(McdDB.TABLE_POPUPS_SCREEN,
							new String[] { McdDB.COLUMN_POPUP_ID,
									McdDB.COLUMN_VIEWROOT_LEVEL },
							McdDB.COLUMN_SCREEN_ID + " ?",
							new String[] { String.valueOf(screenID) }, null,
							null, McdDB.COLUMN_VIEWROOT_LEVEL + " ASC");
					if (!(tmp.moveToFirst())) {
						mcdRaiseException(
								"Android Bug-checker encountered a screen"
										+ " with no associated popup windows",
								database);
					}
					Cursor rootRes;
					...
					if (!(rootRes.moveToFirst())) {
						mcdRaiseException(
								"Android Bug-checker hit a screen with no root view",
								database);
					}
					viewIDLst.add(new Pair<Integer, Integer>(rootRes
							.getInt(rootRes
									.getColumnIndexOrThrow(McdDB.COLUMN_ID)),
							viewRootLimit));
					rootRes.close();
				...
				for (int i = 0; i < viewIDLst.size(); i++) {
					storeCurrentUIEvents(
							WindowManagerImpl.getDefault().getViewRoots()[WindowManagerImpl
									.getDefault().getViewRoots().length
									- viewIDLst.get(i).second].getView(),
							viewIDLst.get(i).first, pathNodeID, database);
				}

				if (getVisibleActivity() != null) {
					addEventToUnexploredList(BACK_CLICK_EVENT_ID, UI_EVENT,
							pathNodeID, KEY_PRESS_EVENT_PRIORITY, database);
					addEventToUnexploredList(MENU_CLICK_EVENT_ID, UI_EVENT,
							pathNodeID, KEY_PRESS_EVENT_PRIORITY, database);
					addEventToUnexploredList(ROTATE_SCREEN_EVENT_ID, UI_EVENT,
							pathNodeID, KEY_PRESS_EVENT_PRIORITY, database);
				}

				int[] nextEvent = selectNextEventToTrigger(pathNodeID,
						ui_envID, database);
				updatePathNodeWithEvent(pathNodeID, nextEvent[0], nextEvent[1],
						database);
				if (nextEvent[1] == -1) {
					// no event to be triggered at this point. so backtrack
					// this also means no event is stored in unexplored_events
					// table for this pathnodeId
					...
					database.delete(McdDB.TABLE_PATH_NODE, McdDB.COLUMN_ID
							+ " = ?",
							new String[] { String.valueOf(pathNodeID) });
					Cursor btCursor = database.rawQuery("SELECT COUNT(*) FROM "
					...
					Log.v(TAG,
							"triggered event on pathnode:"
									+ String.valueOf(pathNodeID));
					// printMessageDetails("\ntriggered event:" +
					// String.valueOf(nextEvent[0]) +
		...
		if (vg != null) {
			for (int i = 0; i < vg.getChildCount(); i++) {
				...
				if (v.isEnabled() && (v.getVisibility() == View.VISIBLE)) {
					...
					if (ViewGroup.class.isInstance(v)) {
						// Log.v("model_checking",
						// "view is an instance of viewGroup");
	...
	public void printMessageDetails(String msg) {
		if (Looper.getMainLooper().getThread() == Thread.currentThread()) {
			...
			// log msg into a file
			File dir = Looper.mcd.getContext().getDir("mcd_dir",
