                           ...
                           Thread* self)
{
    ...
    /*Android bug-checker*/
    /*check if the method belongs to the app, if yes add it to corresponding
     *model checking lists and also set relevant flags in Thread* if this is
     *the first app method in the stack
     */
    if(gDvm.isRunABC == true){
        if((strlen(method->clazz->descriptor) > strlen(gDvm.package_ABC_app)) &&
            ...
            //logic to check if thread is in ABC's map
            ...
            if(it == abcThreadMap.end()){
                ...
                it = abcThreadMap.find(self->abcThreadId);
                if(it != abcThreadMap.end()){
            ...
            /*todo:check if the thread is a binder thread, if yes associate 
             *previous event with it as identity. Also, a HB edge should be added from 
             *previos event (intent) trigger to this

            //we dont do this right now..we have other ways to set connections between environment events and effects             
            if(it != abcThreadMap.end()){
                if(it->second->parentThread == NULL && strncmp("Binder Thread #", 
                    it->second->name, strlen("Binder Thread #")) == 0){
                    LOGE("ABC: attach previous event with this binder thread"); 
                }
            }
            */

            if(abcThreadBaseMethodMap.find(self->threadId) == abcThreadBaseMethodMap.end()){
                abcStoreBaseMethodForThread(self->threadId, method);
            }
            //we track operations only when control is inside an app method
        ...
        if(gDvm.isRunABC == true &&
            ...
            if((strlen(method->clazz->descriptor) > strlen(gDvm.package_ABC_app)) &&
                (strncmp(method->clazz->descriptor, gDvm.package_ABC_app,
                    strlen(gDvm.package_ABC_app)) == 0)){
                isAppMethod = true;
                if(isThreadStackEmpty(self->threadId) && 
                    abcGetBaseMethodForThread(self->threadId) == method){
                    abcRemoveBaseMethodForThread(self->threadId);
                  //  abcRemoveThreadFromStackMap(self->abcThreadId);
                    self->shouldABCTrack = false; 

                    /*each thread whose origin is unknown (e.g., native threads
                     *async tasks etc. are treated as new threads for each of 
                     *their blocks
                     */
                    ...
                    /* ABC todo: this is a scheduling point where you can schedule
                     * an additional thread and let this thread execute as it is
                     * as its no more in App scope 
                     */
                }
            }
            
            //logic to check if shouldABCTrack be set true or false
            const Method* tmpMeth = abcGetLastMethodInThreadStack(self->threadId);
            if(tmpMeth != NULL){
