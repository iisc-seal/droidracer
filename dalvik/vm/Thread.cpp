...
bool dvmCreateInterpThread(Object* threadObj, int reqStackSize)
{
    ...
    /*Android bug-checker*/
    /*Track a thread only if forked from a tracked thread*/
    if(gDvm.isRunABC == true){
        std::map<int, AbcThread*>::iterator parentIter = abcThreadMap.find(self->abcThreadId);
        if(parentIter != abcThreadMap.end() && !parentIter->second->isOriginUntracked){
    //  && abcThreadBaseMethodMap.find(self->threadId) != abcThreadBaseMethodMap.end())
            abcLockMutex(dvmThreadSelf(), &gAbc->abcMainMutex);
            newThread->abcThreadId = abcThreadCount++;
  ...
  //   && abcThreadBaseMethodMap.find(self->threadId) != abcThreadBaseMethodMap.end())
        ...
        if(curAsync->shouldRemove == false && (!curAsync->hasMQ || curAsync->asyncId != -1)){
            //abcSetParentThread(newThread->abcThreadId, self->abcThreadId);
...
static void* interpThreadStart(void* arg)
{
    ...
    /*Android bug-checker*/
    /* This implementation may not be sufficient to detect all 
     * AsyncTasks created in the starting point itself...
     * add to this generic code to detect some special cases
     * like AsyncTasks later.
     */
    if(gDvm.isRunABC == true && self->abcThreadId != -1){
      ...
      //  self->shouldABCTrack = true;
        ...
        /* todo: this is a scheduling point...this should be detected from 
         * the parent thread itself when parent thread's tracking field is 
         * true. Block both parent and child thread after both get into a 
         * runnable state. The scheduler schedules a thread to be executed.
         */
    }else{
        self->abcThreadId = -1;
    }
    ...
    /*Android bug-checker
     * logic to control thread to be executed. We do not need this in
     * current execution
     *
    if(gDvm.isRunABC == true){
        abcCondWait(&self->abcCond, &gAbc->abcMainMutex);
        LOGE("ABC: thread scheduled: %d", self->threadId);
    }
    Android bug-checker*/
...
bool dvmAttachCurrentThread(const JavaVMAttachArgs* pArgs, bool isDaemon)
{
    ...
    /*Android bug-checker*/
    /* We need not track native threads before they access any
     * app method. 
     */
    /*if(gDvm.isRunABC == true){
        dvmThreadSelf()->abcThreadId = abcThreadCount++;
        dvmThreadSelf()->shouldABCTrack = false;
        //abcAddThreadToMap(dvmThreadSelf()->threadId, dvmThreadSelf()->abcThreadId,
        //    pArgs->name);
        abcAddThreadToMap(self, pArgs->name);
